...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.MaterialCalculator = {
                    ...
                    case 'kosebent':
                        ...
                        case 'flans':
                            ...
                            case 'dirsek':
                                ...
                                case 'flansAsme':
                                    console.log('ASME Flanş verileri:', MaterialData.flansVerileriAsme);
                                    
                                    // DN ölçüleri
                                    const dnSizesAsme = ['15', '20', '25', '32', '40', '50', '65', '80', '100', '125', '150', '200', '250', '300'];
                                    const dnOptionsAsme = dnSizesAsme.map(size => 
                                        `<option value="${size}">DN${size}</option>`
                                    ).join('');
                                    
                                    // Class sınıfları
                                    const classTypes = ['Class 150', 'Class 300', 'Class 600', 'Class 900', 'Class 1500', 'Class 2500'];
                                    const classOptions = classTypes.map(cls => 
                                    ...
                                    htmlContent = `
                                        <div class="form-row">
                                            <div class="form-group full-width">
                                                <label for="flansTipiAsme"><span class="label-icon">⚙️</span> Flanş Tipi</label>
                                                <select id="flansTipiAsme" onchange="MaterialCalculator.flansAsmeAgirligiGuncelle()">
                                                    <option value="duzFlansAsme">Düz Flanş (ASME)</option>
                                                    <option value="kaynakBoyunluFlansAsme">Kaynak Boyunlu Flanş (ASME)</option>
                                                    <option value="korFlansAsme">Kör Flanş (ASME)</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="form-group">
                                                <label for="dnOlcusuAsme"><span class="label-icon">📏</span> DN Ölçüsü</label>
                                                <select id="dnOlcusuAsme" onchange="MaterialCalculator.flansAsmeAgirligiGuncelle()">
                                                    ${dnOptionsAsme}
                                                </select>
                                            </div>
                                            <div class="form-group">
                                                <label for="classSinifi"><span class="label-icon">🔧</span> Class Sınıfı</label>
                                                <select id="classSinifi" onchange="MaterialCalculator.flansAsmeAgirligiGuncelle()">
                                                    ${classOptions}
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="form-group full-width">
                                                <label for="flansAsmeAgirlik"><span class="label-icon">⚖️</span> Birim Ağırlık (kg/adet)</label>
                                                <input type="text" id="flansAsmeAgirlik" readonly style="background-color: #f5f5f5;">
                                            </div>
                                        </div>
                                    `;
                                    ...
                                    // İlk yükleme için ağırlığı hesapla
                                    setTimeout(() => {
                                        MaterialCalculator.flansAsmeAgirligiGuncelle();
                                    }, 100);
                                    break;
        ...
        // Flanş ağırlığını güncelle - YENİ FONKSİYON BURAYA EKLE
            ...
            // ASME Flanş ağırlığını güncelle
            flansAsmeAgirligiGuncelle: function() {
                ...
                if (!flansTipiAsme || !dnOlcusuAsme || !classSinifi) return;
                ...
                if (flansDataAsme && flansDataAsme[dnOlcusuAsme] && flansDataAsme[dnOlcusuAsme][classSinifi]) {
                    const agirlik = flansDataAsme[dnOlcusuAsme][classSinifi];
                    document.getElementById('flansAsmeAgirlik').value = agirlik.toFixed(2);
                }
            },
...
window.backupData = async function() {
    const data = {
        projectInfo: TableManager.getProjectInfo(),
        notesRevisions: TableManager.getNotesRevisions(),  // BU SATIRI EKLE

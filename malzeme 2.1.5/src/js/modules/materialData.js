...
(function(window) {
    ...
    // ASME Flanş normları ekle
    FLANS_NORMLARI_ASME: {
        'duzFlansAsme': 'ANSI B 16.5 RF',
        'kaynakBoyunluFlansAsme': 'ANSI B 16.5 WN',
        'korFlansAsme': 'ANSI B 16.5 BL'
    },

    // ASME Flanş tip isimleri
    FLANS_TIPLERI_ASME: {
        'duzFlansAsme': 'Düz Flanş (ASME)',
        'kaynakBoyunluFlansAsme': 'Kaynak Boyunlu Flanş (ASME)',
        'korFlansAsme': 'Kör Flanş (ASME)'
    },
    ...
    // Özel Malzeme Türleri Saklama Yapısı
    ozelMalzemeTurleri: {
        hacimTabanli: {}, // Hacim hesabına göre çalışan malzemeler
        birimAgirlikTabanli: {} // Birim ağırlık ile çalışan malzemeler
    },

    // Özel malzeme türü ekleme - Hacim tabanlı
    ozelMalzemeTuruEkleHacimTabanli: function(malzemeAdi, olcuAlanlari, hesaplamaFormulu) {
        const malzemeKodu = this.generateMalzemeKodu(malzemeAdi);
        
        this.ozelMalzemeTurleri.hacimTabanli[malzemeKodu] = {
            ad: malzemeAdi,
            kod: malzemeKodu,
            tip: 'hacimTabanli',
            olcuAlanlari: olcuAlanlari, // [{id: 'kalinlik', label: 'Kalınlık', birim: 'mm'}, ...]
            hesaplamaFormulu: hesaplamaFormulu, // Hacim hesaplama formülü
            tarih: new Date().toISOString()
        };
        
        this.saveOzelMalzemeler();
        return malzemeKodu;
    },

    // Özel malzeme türü ekleme - Birim ağırlık tabanlı
    ozelMalzemeTuruEkleBirimAgirlikTabanli: function(malzemeAdi, birimAgirlik, olculer, standart, malzemeCinsi) {
        const malzemeKodu = this.generateMalzemeKodu(malzemeAdi);
        
        console.log('Birim ağırlık tabanlı malzeme ekleniyor:', {
            ad: malzemeAdi,
            kod: malzemeKodu,
        ...
        this.ozelMalzemeTurleri.birimAgirlikTabanli[malzemeKodu] = {
            ad: malzemeAdi,
            kod: malzemeKodu,
            tip: 'birimAgirlikTabanli',
            birimAgirlik: parseFloat(birimAgirlik),
            olculer: olculer,
            standart: standart || '-',
            malzemeCinsi: malzemeCinsi || '-',
            tarih: new Date().toISOString()
        ...
        this.saveOzelMalzemeler();
        console.log('Malzeme kaydedildi:', this.ozelMalzemeTurleri.birimAgirlikTabanli[malzemeKodu]);
        return malzemeKodu;
    },

    // Malzeme kodu oluştur
    generateMalzemeKodu: function(malzemeAdi) {
        ...
        let kod = malzemeAdi.toLowerCase();
        for (let [key, value] of Object.entries(turkishChars)) {
            kod = kod.replace(new RegExp(key, 'g'), value);
        }
        
        // Sadece alfanumerik karakterler bırak
        kod = kod.replace(/[^a-z0-9]/g, '_');
        kod = 'ozel_' + kod;
        
        // Eğer kod zaten varsa, sayı ekle
        let sayac = 1;
        let orijinalKod = kod;
        while (this.ozelMalzemeTurleri.hacimTabanli[kod] || this.ozelMalzemeTurleri.birimAgirlikTabanli[kod]) {
            kod = orijinalKod + '_' + sayac;
            sayac++;
        }
    ...
    // Özel malzemeleri kaydet
    saveOzelMalzemeler: function() {
        const data = {
            hacimTabanli: this.ozelMalzemeTurleri.hacimTabanli,
            birimAgirlikTabanli: this.ozelMalzemeTurleri.birimAgirlikTabanli,
            ozelMalzemeCinsleri: this.getCustomMaterials()
        };
        localStorage.setItem('ozelMalzemeler', JSON.stringify(data));
    },

    // Özel malzemeleri yükle
    loadOzelMalzemeler: function() {
        const stored = localStorage.getItem('ozelMalzemeler');
        if (stored) {
            const data = JSON.parse(stored);
            this.ozelMalzemeTurleri.hacimTabanli = data.hacimTabanli || {};
            this.ozelMalzemeTurleri.birimAgirlikTabanli = data.birimAgirlikTabanli || {};
        }
    },

    // Tüm özel malzemeleri getir
    getAllOzelMalzemeler: function() {
        return {
            hacimTabanli: this.ozelMalzemeTurleri.hacimTabanli,
            birimAgirlikTabanli: this.ozelMalzemeTurleri.birimAgirlikTabanli
        };
    ...
    // Özel malzeme türünü sil
    deleteOzelMalzemeTuru: function(kod) {
        if (this.ozelMalzemeTurleri.hacimTabanli[kod]) {
            delete this.ozelMalzemeTurleri.hacimTabanli[kod];
        }
        if (this.ozelMalzemeTurleri.birimAgirlikTabanli[kod]) {
            delete this.ozelMalzemeTurleri.birimAgirlikTabanli[kod];
        }
        this.saveOzelMalzemeler();
    },
    ...
    profilVerileri: {
        ...
        npu: {
            50: { agirlik: 6.81, kesitAlan: 8.68, h: 50, b: 38, tw: 4.5, tf: 7.0 },
            65: { agirlik: 10.9, kesitAlan: 13.9, h: 65, b: 42, tw: 5.5, tf: 8.5 },
    ...
    flansVerileri: {
        ...
        duzFlans: {
            // DN15
            '15': {
                'PN6': 0.5, 'PN10': 0.5, 'PN16': 0.6, 'PN25': 0.7, 'PN40': 0.8, 'PN63': 1.1, 'PN100': 1.5
            },
            // DN20
            '20': {
                'PN6': 0.6, 'PN10': 0.6, 'PN16': 0.7, 'PN25': 0.9, 'PN40': 1.1, 'PN63': 1.5, 'PN100': 2.0
            },
            // DN25
            '25': {
                'PN6': 0.8, 'PN10': 0.8, 'PN16': 0.9, 'PN25': 1.2, 'PN40': 1.5, 'PN63': 2.0, 'PN100': 2.7
            },
            // DN32
            '32': {
                'PN6': 1.0, 'PN10': 1.0, 'PN16': 1.2, 'PN25': 1.6, 'PN40': 2.0, 'PN63': 2.7, 'PN100': 3.8
            },
            // DN40
            '40': {
                'PN6': 1.2, 'PN10': 1.2, 'PN16': 1.4, 'PN25': 1.8, 'PN40': 2.3, 'PN63': 3.1, 'PN100': 4.3
            },
            // DN50
            '50': {
                'PN6': 1.5, 'PN10': 1.5, 'PN16': 1.8, 'PN25': 2.3, 'PN40': 3.0, 'PN63': 4.1, 'PN100': 5.6
            },
            // DN65
            '65': {
                'PN6': 2.0, 'PN10': 2.0, 'PN16': 2.4, 'PN25': 3.2, 'PN40': 4.2, 'PN63': 5.7, 'PN100': 7.9
            },
            // DN80
            '80': {
                'PN6': 2.5, 'PN10': 2.5, 'PN16': 3.0, 'PN25': 4.0, 'PN40': 5.3, 'PN63': 7.3, 'PN100': 10.0
            },
            // DN100
            '100': {
                'PN6': 3.2, 'PN10': 3.5, 'PN16': 4.2, 'PN25': 5.5, 'PN40': 7.5, 'PN63': 10.0, 'PN100': 10.3
            },
            // DN125
            '125': {
                'PN6': 4.5, 'PN10': 5.0, 'PN16': 6.0, 'PN25': 8.0, 'PN40': 10.5, 'PN63': 14.4, 'PN100': 19.8
            },
            // DN150
            '150': {
                'PN6': 5.5, 'PN10': 6.2, 'PN16': 7.5, 'PN25': 10.0, 'PN40': 13.5, 'PN63': 18.5, 'PN100': 25.5
            },
            // DN200
            '200': {
                'PN6': 8.5, 'PN10': 9.5, 'PN16': 11.5, 'PN25': 15.5, 'PN40': 21.0, 'PN63': 28.8, 'PN100': 39.7
            },
            // DN250
            '250': {
                'PN6': 12.5, 'PN10': 14.0, 'PN16': 17.5, 'PN25': 23.5, 'PN40': 32.0, 'PN63': 44.0, 'PN100': 60.5
            },
            // DN300
            '300': {
                'PN6': 16.0, 'PN10': 18.5, 'PN16': 23.0, 'PN25': 31.5, 'PN40': 43.0, 'PN63': 59.1, 'PN100': 81.2
            },
            // DN350
            '350': {
                'PN6': 20.5, 'PN10': 24.0, 'PN16': 30.0, 'PN25': 41.0, 'PN40': 56.0, 'PN63': 77.0, 'PN100': 105.8
            },
            // DN400
            '400': {
                'PN6': 25.0, 'PN10': 29.5, 'PN16': 37.5, 'PN25': 51.5, 'PN40': 71.0, 'PN63': 97.6, 'PN100': 134.2
            },
            // DN500
            '500': {
                'PN6': 37.0, 'PN10': 44.0, 'PN16': 56.5, 'PN25': 78.0, 'PN40': 108.0, 'PN63': 148.5, 'PN100': 204.2
            },
            // DN600
            '600': {
                'PN6': 51.0, 'PN10': 61.0, 'PN16': 79.0, 'PN25': 110.0, 'PN40': 153.0, 'PN63': 210.3, 'PN100': 289.2
            }
        },
        ...
        kaynakBoyunluFlans: {
            '15': {
                'PN6': 0.6, 'PN10': 0.6, 'PN16': 0.7, 'PN25': 0.8, 'PN40': 1.0, 'PN63': 1.1, 'PN100': 1.5
            },
            '20': {
                'PN6': 0.7, 'PN10': 0.7, 'PN16': 0.8, 'PN25': 1.0, 'PN40': 1.3, 'PN63': 1.5, 'PN100': 2.0
            },
            '25': {
                'PN6': 0.9, 'PN10': 0.9, 'PN16': 1.1, 'PN25': 1.4, 'PN40': 1.8, 'PN63': 2.0, 'PN100': 2.7
            },
            '32': {
                'PN6': 1.2, 'PN10': 1.2, 'PN16': 1.4, 'PN25': 1.9, 'PN40': 2.4, 'PN63': 2.7, 'PN100': 3.8
            },
            '40': {
                'PN6': 1.4, 'PN10': 1.4, 'PN16': 1.7, 'PN25': 2.2, 'PN40': 2.8, 'PN63': 3.1, 'PN100': 4.3
            },
            '50': {
                'PN6': 1.8, 'PN10': 1.8, 'PN16': 2.2, 'PN25': 2.8, 'PN40': 3.7, 'PN63': 4.1, 'PN100': 5.6
            },
            '65': {
                'PN6': 2.5, 'PN10': 2.5, 'PN16': 3.0, 'PN25': 4.0, 'PN40': 5.2, 'PN63': 5.7, 'PN100': 7.9
            },
            '80': {
                'PN6': 3.1, 'PN10': 3.1, 'PN16': 3.7, 'PN25': 5.0, 'PN40': 6.6, 'PN63': 7.3, 'PN100': 10.0
            },
            '100': {
                'PN6': 4.0, 'PN10': 4.3, 'PN16': 5.2, 'PN25': 6.9, 'PN40': 9.4, 'PN63': 10.0, 'PN100': 10.3
            },
            '125': {
                'PN6': 5.6, 'PN10': 6.2, 'PN16': 7.5, 'PN25': 10.0, 'PN40': 13.1, 'PN63': 14.4, 'PN100': 19.8
            },
            '150': {
                'PN6': 6.9, 'PN10': 7.7, 'PN16': 9.4, 'PN25': 12.5, 'PN40': 16.9, 'PN63': 18.5, 'PN100': 25.5
            },
            '200': {
                'PN6': 10.6, 'PN10': 11.9, 'PN16': 14.4, 'PN25': 19.4, 'PN40': 26.3, 'PN63': 28.8, 'PN100': 39.7
            },
            '250': {
                'PN6': 15.6, 'PN10': 17.5, 'PN16': 21.9, 'PN25': 29.4, 'PN40': 40.0, 'PN63': 44.0, 'PN100': 60.5
            },
            '300': {
                'PN6': 20.0, 'PN10': 23.1, 'PN16': 28.8, 'PN25': 39.4, 'PN40': 53.8, 'PN63': 59.1, 'PN100': 81.2
            },
            '350': {
                'PN6': 25.6, 'PN10': 30.0, 'PN16': 37.5, 'PN25': 51.3, 'PN40': 70.0, 'PN63': 77.0, 'PN100': 105.8
            },
            '400': {
                'PN6': 31.3, 'PN10': 36.9, 'PN16': 46.9, 'PN25': 64.4, 'PN40': 88.8, 'PN63': 97.6, 'PN100': 134.2
            },
            '500': {
                'PN6': 46.3, 'PN10': 55.0, 'PN16': 70.6, 'PN25': 97.5, 'PN40': 135.0, 'PN63': 148.5, 'PN100': 204.2
            },
            '600': {
                'PN6': 63.8, 'PN10': 76.3, 'PN16': 98.8, 'PN25': 137.5, 'PN40': 191.3, 'PN63': 210.3, 'PN100': 289.2
            }
        },
        ...
        korFlans: {
            '15': {
                'PN6': 0.7, 'PN10': 0.7, 'PN16': 0.8, 'PN25': 1.0, 'PN40': 1.2, 'PN63': 1.1, 'PN100': 1.5
            },
            '20': {
                'PN6': 0.8, 'PN10': 0.8, 'PN16': 1.0, 'PN25': 1.3, 'PN40': 1.6, 'PN63': 1.5, 'PN100': 2.0
            },
            '25': {
                'PN6': 1.1, 'PN10': 1.1, 'PN16': 1.3, 'PN25': 1.7, 'PN40': 2.2, 'PN63': 2.0, 'PN100': 2.7
            },
            '32': {
                'PN6': 1.5, 'PN10': 1.5, 'PN16': 1.8, 'PN25': 2.3, 'PN40': 3.0, 'PN63': 2.7, 'PN100': 3.8
            },
            '40': {
                'PN6': 1.8, 'PN10': 1.8, 'PN16': 2.1, 'PN25': 2.7, 'PN40': 3.5, 'PN63': 3.1, 'PN100': 4.3
            },
            '50': {
                'PN6': 2.2, 'PN10': 2.2, 'PN16': 2.7, 'PN25': 3.5, 'PN40': 4.5, 'PN63': 4.1, 'PN100': 5.6
            },
            '65': {
                'PN6': 3.1, 'PN10': 3.1, 'PN16': 3.7, 'PN25': 4.9, 'PN40': 6.4, 'PN63': 5.7, 'PN100': 7.9
            },
            '80': {
                'PN6': 3.9, 'PN10': 3.9, 'PN16': 4.6, 'PN25': 6.2, 'PN40': 8.1, 'PN63': 7.3, 'PN100': 10.0
            },
            '100': {
                'PN6': 5.4, 'PN10': 5.8, 'PN16': 7.0, 'PN25': 9.2, 'PN40': 12.5, 'PN63': 10.0, 'PN100': 10.3
            },
            '125': {
                'PN6': 7.6, 'PN10': 8.4, 'PN16': 10.1, 'PN25': 13.5, 'PN40': 17.6, 'PN63': 14.4, 'PN100': 19.8
            },
            '150': {
                'PN6': 9.6, 'PN10': 10.8, 'PN16': 13.1, 'PN25': 17.5, 'PN40': 23.6, 'PN63': 18.5, 'PN100': 25.5
            },
            '200': {
                'PN6': 15.2, 'PN10': 17.0, 'PN16': 20.6, 'PN25': 27.7, 'PN40': 37.5, 'PN63': 28.8, 'PN100': 39.7
            },
            '250': {
                'PN6': 23.0, 'PN10': 25.8, 'PN16': 32.2, 'PN25': 43.2, 'PN40': 58.9, 'PN63': 44.0, 'PN100': 60.5
            },
            '300': {
                'PN6': 31.0, 'PN10': 35.8, 'PN16': 44.6, 'PN25': 61.0, 'PN40': 83.3, 'PN63': 59.1, 'PN100': 81.2
            },
            '350': {
                'PN6': 40.5, 'PN10': 47.5, 'PN16': 59.3, 'PN25': 81.2, 'PN40': 110.8, 'PN63': 77.0, 'PN100': 105.8
            },
            '400': {
                'PN6': 50.8, 'PN10': 59.9, 'PN16': 76.1, 'PN25': 104.5, 'PN40': 144.0, 'PN63': 97.6, 'PN100': 134.2
            },
            '500': {
                'PN6': 77.8, 'PN10': 92.4, 'PN16': 118.6, 'PN25': 163.8, 'PN40': 226.9, 'PN63': 148.5, 'PN100': 204.2
            },
            '600': {
                'PN6': 110.5, 'PN10': 132.0, 'PN16': 170.9, 'PN25': 237.9, 'PN40': 331.1, 'PN63': 210.3, 'PN100': 289.2
            }
        }
    },

    // ASME Flanş ağırlık verileri (kg cinsinden) - ANSI B 16.5
    flansVerileriAsme: {
        // Düz Flanş (Raised Face) - ANSI B 16.5
        duzFlansAsme: {
            // DN15 (1/2")
            '15': {
                'Class 150': 0.6, 'Class 300': 0.9, 'Class 600': 1.1, 'Class 900': 2.0, 'Class 1500': 2.0, 'Class 2500': 3.6
            },
            // DN20 (3/4")
            '20': {
                'Class 150': 0.9, 'Class 300': 1.4, 'Class 600': 1.8, 'Class 900': 2.7, 'Class 1500': 2.7, 'Class 2500': 5.0
            },
            // DN25 (1")
            '25': {
                'Class 150': 1.1, 'Class 300': 1.8, 'Class 600': 2.3, 'Class 900': 3.6, 'Class 1500': 3.6, 'Class 2500': 6.8
            },
            // DN32 (1-1/4")
            '32': {
                'Class 150': 1.6, 'Class 300': 2.3, 'Class 600': 3.2, 'Class 900': 5.4, 'Class 1500': 5.4, 'Class 2500': 9.5
            },
            // DN40 (1-1/2")
            '40': {
                'Class 150': 1.8, 'Class 300': 2.7, 'Class 600': 3.6, 'Class 900': 6.4, 'Class 1500': 6.4, 'Class 2500': 11.3
            },
            // DN50 (2")
            '50': {
                'Class 150': 2.7, 'Class 300': 4.1, 'Class 600': 5.4, 'Class 900': 9.5, 'Class 1500': 9.5, 'Class 2500': 18.1
            },
            // DN65 (2-1/2")
            '65': {
                'Class 150': 3.6, 'Class 300': 5.9, 'Class 600': 8.2, 'Class 900': 13.6, 'Class 1500': 13.6, 'Class 2500': 27.2
            },
            // DN80 (3")
            '80': {
                'Class 150': 4.5, 'Class 300': 7.7, 'Class 600': 10.4, 'Class 900': 18.1, 'Class 1500': 20.4, 'Class 2500': 36.3
            },
            // DN100 (4")
            '100': {
                'Class 150': 6.8, 'Class 300': 11.3, 'Class 600': 15.9, 'Class 900': 29.5, 'Class 1500': 31.8, 'Class 2500': 59.0
            },
            // DN125 (5")
            '125': {
                'Class 150': 9.1, 'Class 300': 15.9, 'Class 600': 22.7, 'Class 900': 40.8, 'Class 1500': 45.4, 'Class 2500': 81.6
            },
            // DN150 (6")
            '150': {
                'Class 150': 11.3, 'Class 300': 20.4, 'Class 600': 31.8, 'Class 900': 54.4, 'Class 1500': 63.5, 'Class 2500': 113.4
            },
            // DN200 (8")
            '200': {
                'Class 150': 18.1, 'Class 300': 34.0, 'Class 600': 54.4, 'Class 900': 90.7, 'Class 1500': 108.9, 'Class 2500': 204.1
            },
            // DN250 (10")
            '250': {
                'Class 150': 27.2, 'Class 300': 54.4, 'Class 600': 86.2, 'Class 900': 149.7, 'Class 1500': 181.4, 'Class 2500': 340.2
            },
            // DN300 (12")
            '300': {
                'Class 150': 36.3, 'Class 300': 77.1, 'Class 600': 122.5, 'Class 900': 217.7, 'Class 1500': 272.2, 'Class 2500': 476.3
            }
        },
        
        // Kaynak Boyunlu Flanş (Weld Neck) - ANSI B 16.5
        kaynakBoyunluFlansAsme: {
            '15': {
                'Class 150': 0.7, 'Class 300': 1.1, 'Class 600': 1.4, 'Class 900': 2.3, 'Class 1500': 2.3, 'Class 2500': 4.1
            },
            '20': {
                'Class 150': 1.1, 'Class 300': 1.6, 'Class 600': 2.0, 'Class 900': 3.2, 'Class 1500': 3.2, 'Class 2500': 5.4
            },
            '25': {
                'Class 150': 1.4, 'Class 300': 2.0, 'Class 600': 2.7, 'Class 900': 4.1, 'Class 1500': 4.1, 'Class 2500': 7.3
            },
            '32': {
                'Class 150': 1.8, 'Class 300': 2.7, 'Class 600': 3.6, 'Class 900': 5.9, 'Class 1500': 5.9, 'Class 2500': 10.0
            },
            '40': {
                'Class 150': 2.0, 'Class 300': 3.2, 'Class 600': 4.1, 'Class 900': 6.8, 'Class 1500': 6.8, 'Class 2500': 11.8
            },
            '50': {
                'Class 150': 3.2, 'Class 300': 4.5, 'Class 600': 5.9, 'Class 900': 10.0, 'Class 1500': 10.0, 'Class 2500': 18.6
            },
            '65': {
                'Class 150': 4.1, 'Class 300': 6.4, 'Class 600': 8.6, 'Class 900': 14.1, 'Class 1500': 14.1, 'Class 2500': 27.7
            },
            '80': {
                'Class 150': 5.0, 'Class 300': 8.2, 'Class 600': 10.9, 'Class 900': 18.6, 'Class 1500': 20.9, 'Class 2500': 36.8
            },
            '100': {
                'Class 150': 7.3, 'Class 300': 11.8, 'Class 600': 16.4, 'Class 900': 30.0, 'Class 1500': 32.3, 'Class 2500': 59.5
            },
            '125': {
                'Class 150': 9.5, 'Class 300': 16.4, 'Class 600': 23.2, 'Class 900': 41.3, 'Class 1500': 45.9, 'Class 2500': 82.1
            },
            '150': {
                'Class 150': 11.8, 'Class 300': 20.9, 'Class 600': 32.3, 'Class 900': 54.9, 'Class 1500': 64.0, 'Class 2500': 113.9
            },
            '200': {
                'Class 150': 18.6, 'Class 300': 34.5, 'Class 600': 54.9, 'Class 900': 91.2, 'Class 1500': 109.3, 'Class 2500': 204.6
            },
            '250': {
                'Class 150': 27.7, 'Class 300': 54.9, 'Class 600': 86.7, 'Class 900': 150.1, 'Class 1500': 181.9, 'Class 2500': 340.7
            },
            '300': {
                'Class 150': 36.8, 'Class 300': 77.6, 'Class 600': 123.0, 'Class 900': 218.2, 'Class 1500': 272.7, 'Class 2500': 476.8
            }
        },
        
        // Kör Flanş (Blind) - ANSI B 16.5
        korFlansAsme: {
            '15': {
                'Class 150': 0.9, 'Class 300': 1.4, 'Class 600': 1.8, 'Class 900': 2.7, 'Class 1500': 2.7, 'Class 2500': 5.0
            },
            '20': {
                'Class 150': 1.4, 'Class 300': 2.0, 'Class 600': 2.7, 'Class 900': 3.6, 'Class 1500': 3.6, 'Class 2500': 6.4
            },
            '25': {
                'Class 150': 1.8, 'Class 300': 2.7, 'Class 600': 3.6, 'Class 900': 5.4, 'Class 1500': 5.4, 'Class 2500': 9.1
            },
            '32': {
                'Class 150': 2.3, 'Class 300': 3.6, 'Class 600': 5.0, 'Class 900': 7.7, 'Class 1500': 7.7, 'Class 2500': 13.6
            },
            '40': {
                'Class 150': 2.7, 'Class 300': 4.5, 'Class 600': 5.9, 'Class 900': 9.5, 'Class 1500': 9.5, 'Class 2500': 16.3
            },
            '50': {
                'Class 150': 4.1, 'Class 300': 6.8, 'Class 600': 9.1, 'Class 900': 15.0, 'Class 1500': 15.0, 'Class 2500': 27.2
            },
            '65': {
                'Class 150': 5.4, 'Class 300': 9.5, 'Class 600': 13.6, 'Class 900': 22.7, 'Class 1500': 22.7, 'Class 2500': 40.8
            },
            '80': {
                'Class 150': 6.8, 'Class 300': 13.6, 'Class 600': 18.1, 'Class 900': 31.8, 'Class 1500': 34.0, 'Class 2500': 54.4
            },
            '100': {
                'Class 150': 10.9, 'Class 300': 20.4, 'Class 600': 27.2, 'Class 900': 50.0, 'Class 1500': 54.4, 'Class 2500': 90.7
            },
            '125': {
                'Class 150': 15.0, 'Class 300': 29.5, 'Class 600': 40.8, 'Class 900': 72.6, 'Class 1500': 81.6, 'Class 2500': 136.1
            },
            '150': {
                'Class 150': 18.1, 'Class 300': 38.6, 'Class 600': 59.0, 'Class 900': 99.8, 'Class 1500': 117.9, 'Class 2500': 190.5
            },
            '200': {
                'Class 150': 31.8, 'Class 300': 68.0, 'Class 600': 104.3, 'Class 900': 172.4, 'Class 1500': 208.7, 'Class 2500': 349.3
            },
            '250': {
                'Class 150': 50.0, 'Class 300': 108.9, 'Class 600': 172.4, 'Class 900': 290.3, 'Class 1500': 353.8, 'Class 2500': 589.7
            },
            '300': {
                'Class 150': 68.0, 'Class 300': 158.8, 'Class 600': 249.5, 'Class 900': 435.4, 'Class 1500': 544.3, 'Class 2500': 861.8
            }
        }
    },

...
(function(window) {
    ...
    calculate: function() {
        ...
        try {
            switch(tur) {
                ...
                case 'flansAsme':
                    const flansAsmeResult = this.calculateFlansAsme();
                    birimAgirlik = flansAsmeResult.agirlik;
                    olculer = flansAsmeResult.olculer;
                    break;    
                ...
                case 'eklenenMalzeme':
                    const eklenenResult = this.calculateEklenenMalzeme();
                    birimAgirlik = eklenenResult.agirlik;
                    olculer = eklenenResult.olculer;
                    break;
    ...
    // ASME Flanş hesaplama
    calculateFlansAsme: function() {
        const flansTipiAsme = document.getElementById('flansTipiAsme').value;
        const dnOlcusuAsme = document.getElementById('dnOlcusuAsme').value;
        const classSinifi = document.getElementById('classSinifi').value;
        
        const flansDataAsme = MaterialData.flansVerileriAsme[flansTipiAsme];
        let agirlik = 0;
        
        if (flansDataAsme && flansDataAsme[dnOlcusuAsme] && flansDataAsme[dnOlcusuAsme][classSinifi]) {
            agirlik = flansDataAsme[dnOlcusuAsme][classSinifi];
        }
        
        // DN ve Class bilgisi
        const olculer = `DN${dnOlcusuAsme} ${classSinifi}`;
        
        // Flanş tipi bilgisini de sakla
        this.currentCalculation.flansTipiAsme = flansTipiAsme;
    ...
    // Eklenen malzeme hesaplama
    calculateEklenenMalzeme: function() {
        const secilenMalzeme = document.getElementById('eklenenMalzemeTipi')?.value;
        
        if (!secilenMalzeme) {
            throw new Error('Malzeme seçilmedi');
        }
        
        // Önce hacim tabanlı malzemelerde ara
        const hacimTabanli = MaterialData.ozelMalzemeTurleri.hacimTabanli[secilenMalzeme];
        if (hacimTabanli) {
            return this.calculateHacimTabanliMalzeme(hacimTabanli);
        }
        
        // Birim ağırlık tabanlı malzemelerde ara
        const birimAgirlikTabanli = MaterialData.ozelMalzemeTurleri.birimAgirlikTabanli[secilenMalzeme];
        if (birimAgirlikTabanli) {
            return {
                agirlik: birimAgirlikTabanli.birimAgirlik,
                olculer: birimAgirlikTabanli.olculer
            };
        }
        
        throw new Error('Malzeme verisi bulunamadı');
    },

    // Hacim tabanlı malzeme hesaplama
    calculateHacimTabanliMalzeme: function(malzemeData) {
        const malzemeCinsi = document.getElementById('malzemeCinsi').value;
        const yogunluk = MaterialData.YOGUNLUKLAR[malzemeCinsi] || 7850;
        ...
        let hacim = 0;
        try {
            // Math ve değişkenleri global scope'a geçici olarak ekle
            const originalMath = window.Math;
            const tempVars = {};
            
            // Değişkenleri window'a ekle
            Object.keys(olcuDegerleri).forEach(key => {
                tempVars[key] = window[key]; // Önceki değeri sakla
                window[key] = olcuDegerleri[key];
            });
            
            // Math'i garanti et
            window.Math = Math;
            
            console.log('Formül:', malzemeData.hesaplamaFormulu);
            console.log('Değişkenler:', olcuDegerleri);
            
            // Formülü değerlendir
            hacim = eval(malzemeData.hesaplamaFormulu);
            
            // mm³'den m³'e çevir
            hacim = hacim / 1000000000;
            
            // Geçici değişkenleri temizle
            Object.keys(olcuDegerleri).forEach(key => {
                if (tempVars[key] === undefined) {
                    delete window[key];
                } else {
                    window[key] = tempVars[key];
                }
            });
            
            if (isNaN(hacim) || hacim <= 0) {
                throw new Error('Geçersiz hacim değeri hesaplandı');
            }
            ...
            throw new Error('Hacim hesaplaması başarısız: ' + error.message);
        }
        
        const agirlik = hacim * yogunluk;
        
        // Ölçüleri formatla - Sadece en sonuna birim ekle
        const olcuDegerleri_array = malzemeData.olcuAlanlari.map(alan => olcuDegerleri[alan.id] || 0);
        const birim = malzemeData.olcuAlanlari[0]?.birim || 'mm'; // İlk alanın birimini al
        const olculer = olcuDegerleri_array.join('x') + birim;
    ...
    clearForm: function() {
        ...
        const previousType = document.getElementById('malzemeTuru').value;
        
        document.getElementById('malzemeTuru').value = '';
        document.getElementById('adet').value = '1';
        document.getElementById('heatNo').value = '';
        
        // Ölçü alanlarını temizlerken event listener'ları koru
        const olcuAlanlari = document.getElementById('olcuAlanlari');
        if (olcuAlanlari) {
            // innerHTML yerine childNodes'ları temizle
            while (olcuAlanlari.firstChild) {
                olcuAlanlari.removeChild(olcuAlanlari.firstChild);
            }
        }
        ...
        // EventManager'ı bilgilendir
        if (window.EventManager) {
            setTimeout(() => {
                window.EventManager.reactivateAllInputs();
            }, 10);
        }
    },

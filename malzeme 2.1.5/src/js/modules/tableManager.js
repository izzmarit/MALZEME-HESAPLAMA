...
(function(window) {
    ...
    addRow: function() {
        ...
        const malzemeTuruFormatlari = {
            ...
            'npu': 'NPU Profil',
            'npi': 'NPI Profil',
            'flans': 'Standart Flanş (EN)',
            'flansAsme': 'Standart Flanş (ASME)',
        ...
        // ASME Flanş kontrolü ve değer ataması
        else if (tur === 'flansAsme') {
            ...
            malzemeTuruGosterim = MaterialData.FLANS_TIPLERI_ASME[flansTipiAsme];
            malzemeCinsi = this.formatMalzemeCinsiForDisplay(document.getElementById('malzemeCinsi').value);
            enNormu = 'ANSI B 16.5';
            olculer = Calculator.currentCalculation.olculer;
            birimAgirlik = parseFloat(document.getElementById('birimAgirlik').textContent);
        ...
        if (tur === 'flans') {
            ...
            rowData.flansTipiAsme = document.getElementById('flansTipiAsme')?.value;
        } else if (tur === 'izgara') {
                ...
                <td>
                    <div class="table-action-buttons">
                        ...
                        <button class="btn-revision ${row.isRevision ? 'active' : ''}" onclick="TableManager.toggleRevision(${index})">
                            <span data-lang="revision">Revizyon</span>
                        </button>
    ...
    toggleRevision: function(index) {
        if (this.tableData[index]) {
            // Revizyon durumunu tersine çevir
        ...
        this.tableData.forEach(row => {
            ...
            const typeTranslations = {
                'tr': {
                    ...
                    'Kör Flanş': 'Kör Flanş',
                    'Düz Flanş (ASME)': 'Düz Flanş (ASME)',
                    'Kaynak Boyunlu Flanş (ASME)': 'Kaynak Boyunlu Flanş (ASME)',
                    'Kör Flanş (ASME)': 'Kör Flanş (ASME)',
                ...
                'en': {
                    ...
                    'Düz Flanş (ASME)': 'Flat Flange (ASME)',
                    'Kaynak Boyunlu Flanş (ASME)': 'Weld Neck Flange (ASME)',
                    'Kör Flanş (ASME)': 'Blind Flange (ASME)',
    ...
    fillDimensionsFromRow: function(row) {
        ...
        switch(tur) {
            ...
            case 'flansAsme':
                if (row.flansTipiAsme) {
                    document.getElementById('flansTipiAsme').value = row.flansTipiAsme;
                    // DN ve Class değerlerini çıkar
                    const flansAsmeMatch = olculer.match(/DN(\d+)\s+(Class\s+\d+)/);
                    if (flansAsmeMatch) {
                        document.getElementById('dnOlcusuAsme').value = flansAsmeMatch[1];
                        document.getElementById('classSinifi').value = flansAsmeMatch[2];
                    }
                    // Ağırlığı güncelle
                    setTimeout(() => MaterialCalculator.flansAsmeAgirligiGuncelle(), 100);
                }
                break;    
    ...
    updateRow: function() {
        ...
        if (tur === 'ozelMalzeme') {
            ...
            this.tableData[index] = {
                ...
                isRevision: this.tableData[index].isRevision || false  // Revizyon durumunu koru
            };
        }
        else if (tur === 'izgara') {
            const izgaraTipi = document.getElementById('izgaraTipi')?.value;
        ...
        // Standart Flanş kontrolü ve değer ataması
                ...
                if (!flansTipi) {
                    UIManager.showNotification('Lütfen flanş tipi seçin!', 'warning');
                    return;
                }
                
                // Flanş ağırlığını hesapla
                if (!Calculator.calculate()) {
                ...
                this.tableData[index] = {
                    ...this.tableData[index],
                    adet: adet,
                    malzemeTuru: MaterialData.FLANS_TIPLERI[flansTipi],
                    malzemeCinsi: this.formatMalzemeCinsiForDisplay(document.getElementById('malzemeCinsi').value),
                    olculer: Calculator.currentCalculation.olculer,
                    enNormu: MaterialData.FLANS_NORMLARI[flansTipi],
                    birimAgirlik: Calculator.currentCalculation.birimAgirlik.toFixed(2),
                    ...
                    originalGrade: document.getElementById('malzemeCinsi').value,
                    flansTipi: flansTipi,
                    isRevision: this.tableData[index].isRevision || false
                };
            }
            ...
            // ASME Flanş kontrolü ve değer ataması
                ...
                this.tableData[index] = {
                    ...this.tableData[index],
                    adet: adet,
                    malzemeTuru: MaterialData.FLANS_TIPLERI_ASME[flansTipiAsme],
                    malzemeCinsi: this.formatMalzemeCinsiForDisplay(document.getElementById('malzemeCinsi').value),
                    olculer: Calculator.currentCalculation.olculer,
                    enNormu: 'ANSI B 16.5',
                    birimAgirlik: Calculator.currentCalculation.birimAgirlik.toFixed(2),
                    ...
                    heatNo: document.getElementById('heatNo').value || '-',
                    suHacmi: '-',
                    originalType: 'flansAsme',
                    originalGrade: document.getElementById('malzemeCinsi').value,
                    flansTipiAsme: flansTipiAsme,
                    isRevision: this.tableData[index].isRevision || false
                };
            }
            ...
            this.tableData[index] = {
                ...
                flansTipiAsme: tur === 'flansAsme' ? document.getElementById('flansTipiAsme')?.value : null,
                isRevision: this.tableData[index].isRevision || false  // Revizyon durumunu koru
            };
        }

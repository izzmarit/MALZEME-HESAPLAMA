...
function createMenu() {
    const menuTemplate = [
        ...
        {
            label: 'Özel Malzemeler',
            submenu: [
                {
                    label: 'Yeni Malzeme Türü Ekle',
                    click: () => {
                        mainWindow.webContents.send('add-custom-material-type');
                    }
                },
                {
                    label: 'Özel Malzemeleri Yönet',
                    click: () => {
...
// Excel kaydetme dialogu
ipcMain.handle('save-excel-dialog', async (event, options) => {
    const result = await dialog.showSaveDialog(mainWindow, {
        title: 'Excel Dosyasını Kaydet',
        defaultPath: path.join(app.getPath('documents'), options.dosyaAdi + '.xlsx'),
        filters: [

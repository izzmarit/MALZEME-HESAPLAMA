...
(function(window) {
    ...
    class BaseMaterial {
        ...
        editFromTable(rowData) {
            ...
            // FormData varsa önce onu doldur
            if (rowData.formData) {
                Object.keys(rowData.formData).forEach(key => {
                    const element = document.getElementById(key);
                    if (element && key !== 'malzemeTuru') {
            ...
            // Sonra rowData'dan eksik bilgileri doldur
            const fieldsToFill = {
                'adet': rowData.adet,
                'heatNo': rowData.heatNo === '-' ? '' : rowData.heatNo,
                'malzemeCinsi': rowData.originalGrade || rowData.malzemeCinsi
            };
            
            Object.keys(fieldsToFill).forEach(key => {
                const element = document.getElementById(key);
                if (element && fieldsToFill[key] !== undefined) {
                    element.value = fieldsToFill[key];
                }
            });
            
            // Malzeme tipine özgü alanları doldur
            this.fillSpecificFields(rowData);
        ...
        fillSpecificFields(rowData) {
            // Alt sınıflar kendi özel alanlarını doldurur
        }

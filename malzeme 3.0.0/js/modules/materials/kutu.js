...
(function(window) {
    ...
    class KutuMaterial extends window.BaseMaterial {
        ...
        fillSpecificFields(rowData) {
            // ÖNCELİK 0: Metadata'dan gelen kutuDetay
            if (rowData.kutuDetay) {
                console.log('Kutu metadata kutuDetay bulundu:', rowData.kutuDetay);
                
                ['en', 'boy', 'kalinlik', 'uzunluk'].forEach(field => {
                    const element = document.getElementById(field);
                    if (element && rowData.kutuDetay[field] !== undefined) {
                        element.value = rowData.kutuDetay[field];
                    }
                });
                return;
            }
            if (rowData.olculer) {
                ...
                const pattern = /(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)\s*L:\s*(\d+(?:\.\d+)?)/i;
            ...
            if (rowData.formData) {
                const fields = ['en', 'boy', 'kalinlik', 'uzunluk'];

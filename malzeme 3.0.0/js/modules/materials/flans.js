...
(function(window) {
    ...
    class FlansMaterial extends window.BaseMaterial {
        ...
        fillSpecificFields(rowData) {
            console.log('Flanş fillSpecificFields çalıştı:', rowData);
            
            // *** YENİ: ÖNCELİK 0 - Metadata'dan gelen flansDetay ***
            if (rowData.flansDetay && rowData.flansDetay.tipi) {
                console.log('Flanş metadata flansDetay bulundu:', rowData.flansDetay);
                ...
                return; // Metadata varsa diğer kontrollere gerek yok
            }
            
            // İKİNCİL: formData varsa onu kullan
            if (rowData.formData && rowData.formData.flansTipi) {
            ...
            // SON ÇARE: Standart ve ölçülerden parse et
            const enNormu = rowData.enNormu || '';
            const olculer = rowData.olculer || '';
            const malzemeTuru = rowData.malzemeTuru || '';
            ...
            if (!standartSistem) {
                console.log('Flanş standart sistemi belirlenemedi');
                return;
            }
            
            // Flanş tipini malzeme türünden çıkar
            const malzemeTuruLower = malzemeTuru.toLowerCase();
            let flansTipi = '';
            ...
            if (!flansTipi) {
                console.log('Flanş tipi belirlenemedi');
                return;
            }
            ...
            if (tipiElement) {
                ...
                setTimeout(() => {
                    ...
                    if (standartElement) {
                        ...
                        setTimeout(() => {
                            // Format: "DN200 PN40" veya "NPS8 Class 150"
                            let boyutPattern, basincPattern, boyutValue, basincValue;
                            
                            if (standartSistem === 'en') {
                                boyutPattern = /DN\s*(\d+)/i;
                                basincPattern = /PN\s*(\d+)/i;
                            } else { // asme
                                boyutPattern = /NPS\s*([^\s]+)/i;
                                basincPattern = /Class\s*(\d+)/i;
                            }

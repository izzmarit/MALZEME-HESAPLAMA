...
(function(window) {
    ...
    class DirsekMaterial extends window.BaseMaterial {
        ...
        fillSpecificFields(rowData) {
            // ÖNCELİKLE: dirsekDetay varsa
            if (rowData.dirsekDetay) {
            ...
            // YOKSA: Ölçülerden parse et
            if (rowData.olculer) {
                const olculer = rowData.olculer.toString();
                
                // Format: "90° DN200 WT:10mm"
                const aciPattern = /(\d+)°/;
                const boyutPattern = /(?:DN|NPS\s*)(\d+)/i;
                        ...
                        setTimeout(() => {
                            ...
                            if (aciElement) {
                                ...
                                setTimeout(() => {
                                    const boyutElement = document.getElementById('dirsekBoyut');
                                    if (boyutElement) {
                                        boyutElement.value = boyutMatch[1];
                                        window.ApplicationController.updateDirsekThickness();
            ...
            // FormData varsa öncelikli olarak kullan
            if (rowData.formData) {
                console.log('Dirsek formData bulundu:', rowData.formData);
                
                // Standart sistemi ayarla
                const standartElement = document.getElementById('standartSistem');
                if (standartElement && rowData.formData.standartSistem) {
                    ...
                    setTimeout(() => {
                        // Dirsek açısını ayarla
                        const aciElement = document.getElementById('dirsekAci');
                        if (aciElement && rowData.formData.dirsekAci) {
            ...
            // dirsekDetay varsa onu kullan
            if (rowData.dirsekDetay) {
                console.log('Dirsek dirsekDetay bulundu:', rowData.dirsekDetay);
                
                const standartElement = document.getElementById('standartSistem');
                if (standartElement && rowData.dirsekDetay.standartSistem) {
                    standartElement.value = rowData.dirsekDetay.standartSistem;
                    window.ApplicationController.updateDirsekStandards();

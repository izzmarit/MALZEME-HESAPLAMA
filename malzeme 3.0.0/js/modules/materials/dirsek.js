...
(function(window) {
    ...
    class DirsekMaterial extends window.BaseMaterial {
        ...
        fillSpecificFields(rowData) {
            console.log('Dirsek fillSpecificFields çalıştı:', rowData);
            
            // ÖNCELİKLE: dirsekDetay varsa onu kullan (en güvenilir kaynak)
            if (rowData.dirsekDetay && rowData.dirsekDetay.aci) {
                console.log('Dirsek dirsekDetay bulundu:', rowData.dirsekDetay);
                
                // Standart sistemini belirle
                let standartSistem = '';
                if (rowData.enNormu) {
                    if (rowData.enNormu.includes('EN 10253')) {
                        standartSistem = 'en';
                    } else if (rowData.enNormu.includes('ASME B16.9')) {
                        standartSistem = 'asme';
                    }
                }
                
                const standartElement = document.getElementById('standartSistem');
                if (standartElement && standartSistem) {
                    standartElement.value = standartSistem;
                    window.ApplicationController.updateDirsekStandards();
                    
                    setTimeout(() => {
                        const aciElement = document.getElementById('dirsekAci');
                        if (aciElement && rowData.dirsekDetay.aci) {
                            aciElement.value = rowData.dirsekDetay.aci;
                            window.ApplicationController.updateDirsekSizes();
                            
                            setTimeout(() => {
                                const boyutElement = document.getElementById('dirsekBoyut');
                                if (boyutElement && rowData.dirsekDetay.boyut) {
                                    boyutElement.value = rowData.dirsekDetay.boyut;
                                    window.ApplicationController.updateDirsekThickness();
                                    
                                    setTimeout(() => {
                                        const kalinlikElement = document.getElementById('dirsekKalinlik');
                                        if (kalinlikElement && rowData.dirsekDetay.kalinlik) {
                                            kalinlikElement.value = rowData.dirsekDetay.kalinlik;
                                            window.ApplicationController.updateDirsekWeight();
                                        }
                                    }, 200);
                                }
                            }, 200);
                ...
                return; // dirsekDetay varsa diğer kontrollere gerek yok
            }
            
            // İKİNCİL: formData varsa onu kullan
            if (rowData.formData && rowData.formData.dirsekAci) {
                console.log('Dirsek formData bulundu:', rowData.formData);
                ...
                if (standartElement && rowData.formData.standartSistem) {
                    ...
                    setTimeout(() => {
                        const aciElement = document.getElementById('dirsekAci');
                        if (aciElement) {
                            aciElement.value = rowData.formData.dirsekAci;
                            window.ApplicationController.updateDirsekSizes();
                            
                            setTimeout(() => {
                                const boyutElement = document.getElementById('dirsekBoyut');
                                if (boyutElement && rowData.formData.dirsekBoyut) {
                                    boyutElement.value = rowData.formData.dirsekBoyut;
                                    window.ApplicationController.updateDirsekThickness();
                                    
                                    setTimeout(() => {
                                        const kalinlikElement = document.getElementById('dirsekKalinlik');
                                        if (kalinlikElement && rowData.formData.dirsekKalinlik) {
                                            kalinlikElement.value = rowData.formData.dirsekKalinlik;
                                            window.ApplicationController.updateDirsekWeight();
                                        }
                                    }, 200);
                                }
                ...
                return; // formData varsa parse etmeye gerek yok
            }
            
            // SON ÇARE: Ölçülerden parse et
            if (rowData.olculer) {
                const olculer = rowData.olculer.toString();
                
                // Format: "90° DN200 WT:10mm" veya "90° NPS8 WT:10mm"
                const aciPattern = /(\d+)°/;
                const boyutPattern = /(?:DN|NPS\s*)(\d+|[\d\/\-]+)/i;
                const kalinlikPattern = /WT:\s*(\d+(?:\.\d+)?)/i;
                
                const aciMatch = olculer.match(aciPattern);
                const boyutMatch = olculer.match(boyutPattern);
                const kalinlikMatch = olculer.match(kalinlikPattern);
                ...
                // Veya ölçülerden anla
                if (!standartSistem) {
                    if (olculer.includes('DN')) {
                        standartSistem = 'en';
                    } else if (olculer.includes('NPS')) {
                        standartSistem = 'asme';
                    }
                }
                
                if (standartSistem && aciMatch && boyutMatch && kalinlikMatch) {
                    const standartElement = document.getElementById('standartSistem');
                    if (standartElement) {
                        standartElement.value = standartSistem;
                        window.ApplicationController.updateDirsekStandards();
                        ...
                        setTimeout(() => {
                            const aciElement = document.getElementById('dirsekAci');
                            if (aciElement) {
                                aciElement.value = aciMatch[1];
                                window.ApplicationController.updateDirsekSizes();
                                
                                setTimeout(() => {
                                    let boyutValue = boyutMatch[1];
                                    
                                    // ASME için inch notasyonunu DN karşılığına çevir
                                    ...
                                    if (boyutElement) {
                                        ...
                                        setTimeout(() => {
                                            const kalinlikElement = document.getElementById('dirsekKalinlik');
                                            if (kalinlikElement) {
                                                kalinlikElement.value = kalinlikMatch[1];
                                                window.ApplicationController.updateDirsekWeight();
                                            }
                                        }, 200);
                                    }
        ...
        window.ApplicationController.updateDirsekSizes = function() {
            ...
            sizes.forEach(size => {
                ...
                if (standartSistem === 'en') {
                    // EN sisteminde DN değerine karşılık gelen çap
                    const dnCapMap = {
                        '15': '21.3', '20': '26.9', '25': '33.7', '32': '42.4',
                        '40': '48.3', '50': '60.3', '65': '76.1', '80': '88.9',
                        '100': '114.3', '125': '139.7', '150': '168.3', '200': '219.1',
                        '250': '273', '300': '323.9', '350': '355.6', '400': '406.4',
                        '450': '457', '500': '508', '600': '610'
                    };
                    capBilgisi = dnCapMap[size] ? ` (Ø${dnCapMap[size]}mm)` : '';
                }

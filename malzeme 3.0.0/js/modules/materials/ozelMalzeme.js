...
(function(window) {
    ...
    class OzelMalzemeMaterial extends window.BaseMaterial {
        ...
        formatDimensions(formData) {
            ...
            if (malzemeTipiSecim === 'hacim') {
                const parts = [];
                if (formData.olcu1) parts.push(`${formData.olcu1_label || 'Ö1'}:${formData.olcu1}`);
                if (formData.olcu2) parts.push(`${formData.olcu2_label || 'Ö2'}:${formData.olcu2}`);
                if (formData.olcu3) parts.push(`${formData.olcu3_label || 'Ö3'}:${formData.olcu3}`);
                if (formData.olcu4) parts.push(`${formData.olcu4_label || 'Ö4'}:${formData.olcu4}`);
                return `${malzemeAdi} (${parts.join(' x ')})`;
            } else {
                return `${malzemeAdi} (${formData.birimSecim || 'Özel'})`;
            }
        }

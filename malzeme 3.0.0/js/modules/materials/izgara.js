    ...
    class IzgaraMaterial extends window.BaseMaterial {
        ...
        formatDimensions(formData) {
            ...
            if (kategori === 'platform') {
                const en = parseFloat(formData.en) || 0;
                const boy = parseFloat(formData.boy) || 0;
                // Platform ızgaralar için sadece ölçüleri göster
                if (en && boy) {
                    return `${en}x${boy}mm`;
                }
                return '-';
        ...
        formatRow(formData, calculation) {
            ...
            // Kategori ve tip bilgilerini sakla
            baseRow.izgaraKategori = formData.izgaraKategori;
            baseRow.izgaraTipi = formData.izgaraTipi;
            
            // Malzeme türünü güncelle
            if (formData.izgaraKategori === 'platform') {
                // Platform ızgara için tip bilgisini de ekle
                const izgaraData = this.platformIzgaralar[formData.izgaraTipi];
                if (izgaraData) {
                    baseRow.malzemeTuru = formData.izgaraTipi; // "Izgara 30x30x3" formatında
                } else {
                    baseRow.malzemeTuru = 'Platform Izgara';
                }
            } else if (formData.izgaraKategori === 'element') {
        ...
        fillSpecificFields(rowData) {
            // Izgara kategorisi ve tipini doldur
            if (rowData.izgaraKategori) {
                const kategoriElement = document.getElementById('izgaraKategori');
                if (kategoriElement) {
                    kategoriElement.value = rowData.izgaraKategori;
                    window.ApplicationController.updateIzgaraTypes();
                    ...
                    setTimeout(() => {
                        ...
                        // Platform ızgara ise ölçüleri doldur
                        if (rowData.izgaraKategori === 'platform') {
                            if (rowData.formData) {
                                const enElement = document.getElementById('en');
                                const boyElement = document.getElementById('boy');
                                
                                if (enElement && rowData.formData.en) {
                                    enElement.value = rowData.formData.en;
                                }
                                if (boyElement && rowData.formData.boy) {
                                    boyElement.value = rowData.formData.boy;
                                }
                            } else if (rowData.olculer) {
                                // Ölçüleri parse et

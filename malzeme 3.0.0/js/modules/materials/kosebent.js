...
(function(window) {
    ...
    class KosebentMaterial extends window.BaseMaterial {
        ...
        formatRow(formData, calculation) {
            ...
            // Köşebent detaylarını sakla
            baseRow.kosebentDetay = {
                kol1: formData.kol1,
                kol2: formData.kol2,
        ...
        fillSpecificFields(rowData) {
            // ÖNCELİK 0: Metadata'dan gelen kosebentDetay
            if (rowData.kosebentDetay) {
                console.log('Köşebent metadata kosebentDetay bulundu:', rowData.kosebentDetay);
                
                ['kol1', 'kol2', 'kalinlik', 'uzunluk'].forEach(field => {
                    const element = document.getElementById(field);
                    if (element && rowData.kosebentDetay[field] !== undefined) {
                        element.value = rowData.kosebentDetay[field];
                    }
                });
                return;
            }
            // Ölçüleri parse et - formatı: "L 50x50x5 L:6000mm" veya "L 60x40x5 L:6000mm"
            if (rowData.olculer) {
                const olculer = rowData.olculer.toString();
                const pattern = /L?\s*(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)\s*L:\s*(\d+(?:\.\d+)?)/i;
                const matches = olculer.match(pattern);
                
                if (matches) {
                    const kol1Element = document.getElementById('kol1');
                    const kol2Element = document.getElementById('kol2');
                    const kalinlikElement = document.getElementById('kalinlik');
                    const uzunlukElement = document.getElementById('uzunluk');
                    
                    if (kol1Element) kol1Element.value = matches[1];
                    if (kol2Element) kol2Element.value = matches[2];
                    if (kalinlikElement) kalinlikElement.value = matches[3];
                    if (uzunlukElement) uzunlukElement.value = matches[4];
                }
            }
            
            // FormData'dan doldur
            if (rowData.formData) {
                const fields = ['kol1', 'kol2', 'kalinlik', 'uzunluk'];

...
(function(window) {
    ...
    class OzelFlansMaterial extends window.BaseMaterial {
        ...
        fillSpecificFields(rowData) {
            ...
            if (rowData.flansDetay) {
                Object.keys(rowData.flansDetay).forEach(key => {
                    const elementId = fieldsMapping[key] || key;
            ...
            if (rowData.formData) {
                Object.keys(fieldsMapping).forEach(key => {
                    const elementId = fieldsMapping[key] || key;
                    const element = document.getElementById(elementId);
                    if (element && rowData.formData[elementId] !== undefined) {
                        element.value = rowData.formData[elementId];
                    }
                });
                
                if (rowData.formData.flansAdi && !document.getElementById('flansAdi').value) {
                    document.getElementById('flansAdi').value = rowData.formData.flansAdi;
                }
            }
            ...
            const calculatedWeight = instance.calculate(rowData.formData);
            if (calculatedWeight) {
                instance.updateInfoDisplay(calculatedWeight.birimAgirlik);
            }
        }
    }
    ...
    if (!window.ApplicationController.loadFlansTemplate) {
        ...
        window.ApplicationController.saveFlansAsTemplate = function() {
            ...
            if (!flansAdiElement) {
                alert('Flanş adı alanı bulunamadı');
                return;
            }

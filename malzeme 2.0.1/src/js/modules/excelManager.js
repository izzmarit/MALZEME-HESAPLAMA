    ...
    const ExcelManager = {
        ...
        importFromExcel: async function() {
            try {
                ...
                console.log('Özet bölümü aranıyor, başlangıç satırı:', summaryStartRow);

                // Özet bölümünü tara
                for (let i = summaryStartRow; i <= summaryStartRow + 15; i++) {
                    const cellH = worksheet.getCell(rowIndex, 8).value;
                    if (!pNo || (cellH && cellH.toString().toUpperCase().includes('ÖZET'))) {
                        break;
                    }
                    ...
                    if (cellH && cellH.toString().toUpperCase().includes('HAZIRLAYAN')) {
                        ...
                        console.log('Hazırlayan bulundu:', hazirlayan);
                    }
                    ...
                    if (cellH && cellH.toString().toUpperCase().includes('KONTROL') && 
                        ...
                        console.log('Kontrol bulundu:', kontrol);
                    }
                    
                    // Notlar başlığı kontrol (A sütunu)
                    const cellA = worksheet.getCell(i, 1).value;
                    if (cellA && cellA.toString().toLowerCase().includes('notlar')) {
                        // Notlar içeriğini oku (bir sonraki satırdan başlayan birleştirilmiş hücre)
                        const notlarContent = worksheet.getCell(i + 1, 1).value;
                        notlar = notlarContent || '';
                        console.log('Notlar bulundu:', notlar);
                    }
                    
                    // Revizyonlar başlığı kontrol (D sütunu - yeni konumu)
                    const cellD = worksheet.getCell(i, 4).value;
                    if (cellD && cellD.toString().toLowerCase().includes('revizyon')) {
                        // Revizyonlar içeriğini oku (bir sonraki satırdan başlayan birleştirilmiş hücre)
                        const revizyonlarContent = worksheet.getCell(i + 1, 4).value;
                        revizyonlar = revizyonlarContent || '';
                        console.log('Revizyonlar bulundu:', revizyonlar);
                    }
                }

                // Proje bilgilerine ekle
                ...
                console.log('Excel import: Toplam satır sayısı:', tableData.length);

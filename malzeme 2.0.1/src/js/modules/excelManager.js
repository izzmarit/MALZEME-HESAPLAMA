...
(function(window) {
    ...
    const ExcelManager = {
        ...
        addSummarySection: function(worksheet, tableData, startRow) {
            ...
            // Özet bölümü birleştirmeleri - YAN YANA DÜZENLEME
            worksheet.mergeCells(`A${summaryStartRow + 1}:A${summaryStartRow + 1}`); // Notlar başlığı
            worksheet.mergeCells(`A${summaryStartRow + 2}:D${summaryStartRow + 4}`); // Notlar içeriği
            
            worksheet.mergeCells(`E${summaryStartRow + 1}:E${summaryStartRow + 1}`); // Revizyonlar başlığı
            worksheet.mergeCells(`E${summaryStartRow + 2}:G${summaryStartRow + 4}`); // Revizyonlar içeriği
            ...
            // Notlar başlığı (A sütunu)
            const notlarBaslikCell = worksheet.getCell(`A${summaryStartRow + 1}`);
            ...
            // Notlar içeriği (A-D birleşik)
            const notlarIcerikCell = worksheet.getCell(`A${summaryStartRow + 2}`);
            notlarIcerikCell.value = notlarRevizyon.notlar || '';
            notlarIcerikCell.font = { name: 'Calibri', size: 11, bold: false };
            notlarIcerikCell.alignment = { horizontal: 'left', vertical: 'top', wrapText: true };
            
            // Revizyonlar başlığı (E sütunu)
            const revizyonBaslikCell = worksheet.getCell(`E${summaryStartRow + 1}`);
            ...
            // Revizyonlar içeriği (E-G birleşik)
            const revizyonIcerikCell = worksheet.getCell(`E${summaryStartRow + 2}`);
            revizyonIcerikCell.value = notlarRevizyon.revizyonlar || '';
            revizyonIcerikCell.font = { name: 'Calibri', size: 11, bold: false };
            revizyonIcerikCell.alignment = { horizontal: 'left', vertical: 'top', wrapText: true };
            
            // Özet bölümü başlıkları
            const summaryData = [
                { cell: `H${summaryStartRow}`, value: LanguageManager.getText('summary_report', 'ÖZET RAPORU').toUpperCase(), bold: true, fill: true },
        ...
        // Özet bölümü kenarlıkları - YAN YANA DÜZENLİ
        ...
        importFromExcel: async function() {
            try {
                ...
                const projectInfo = {
                    ...
                    hazirlayan: '', // Excel'den sonra özet bölümünden okunacak
                    kontrol: '' // Excel'den sonra özet bölümünden okunacak
                };
                ...
                while (rowIndex <= worksheet.rowCount) {
                    ...
                    // Özel malzemeleri kontrol et ve yükle
                    const hiddenSheet = workbook.getWorksheet('OzelMalzemeler');
                    if (hiddenSheet) {
                        console.log('Özel malzemeler bulundu, yükleniyor...');
                        ...
                        // Mevcut özel malzemeleri yükle
                        MaterialData.ozelMalzemeTurleri.hacimTabanli = {
                            ...MaterialData.ozelMalzemeTurleri.hacimTabanli,
                            ...hacimTabanliMalzemeler
                        };
                        
                        MaterialData.ozelMalzemeTurleri.birimAgirlikTabanli = {
                            ...MaterialData.ozelMalzemeTurleri.birimAgirlikTabanli,
                            ...birimAgirlikTabanliMalzemeler
                        };
                        
                        // Malzeme cinslerini yükle
                        malzemeCinsleri.forEach(material => {
                            if (!MaterialData.YOGUNLUKLAR[material.kod]) {
                        ...
                        this.showNotification('Özel malzemeler yüklendi', 'info');
                    }
                ...
                // Özet bölümünü bul ve notlar/revizyonlar/hazirlayan/kontrol bilgilerini oku
                let summaryStartRow = rowIndex + 1; // Veri bitişinden sonraki satır
                let notlar = '';
                let revizyonlar = '';
                let hazirlayan = '';
                let kontrol = '';
                ...
                // Özet bölümünü tara
                for (let i = summaryStartRow; i <= summaryStartRow + 15; i++) {
                    const cellH = worksheet.getCell(i, 8).value;
                    
                    // Hazirlayan kontrol
                    if (cellH && cellH.toString().toUpperCase().includes('HAZIRLAYAN')) {
                        hazirlayan = worksheet.getCell(i, 10).value || '';
                    }
                    
                    // Kontrol kontrol
                    if (cellH && cellH.toString().toUpperCase().includes('KONTROL') && 
                        !cellH.toString().toUpperCase().includes('HAZIRLAYAN')) {
                        kontrol = worksheet.getCell(i, 10).value || '';
                    }
                    
                    // Notlar başlığı kontrol
                    const cellA = worksheet.getCell(i, 1).value;
                    if (cellA && cellA.toString().toLowerCase().includes('notlar:')) {
                        // Notlar içeriğini oku (bir sonraki birleştirilmiş hücre)
                        notlar = worksheet.getCell(i + 1, 1).value || '';
                    }
                    
                    // Revizyonlar başlığı kontrol
                    if (cellA && cellA.toString().toLowerCase().includes('revizyon')) {
                        // Revizyonlar içeriğini oku (bir sonraki birleştirilmiş hücre)
                        revizyonlar = worksheet.getCell(i + 1, 1).value || '';
                    }
                }

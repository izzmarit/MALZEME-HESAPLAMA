 ...
 * Versiyon: 1.0.0
 * Kutu profil (Kare/Dikdörtgen kesitli kapalı profil) hesaplama modülü - EN standartları ile
 */

(function(window) {
    'use strict';
    
    class KutuProfilMaterial extends window.BaseMaterial {
        constructor() {
            super();
            this.materialType = 'kutuProfil';
            this.version = '1.0.0';
            ...
            this.texts = {
                tr: {
                    ...
                    validation_thickness_error: 'Kalınlık, en ve yükseklik değerlerinin yarısından küçük olmalıdır',
                ...
                en: {
                    ...
                    validation_thickness_error: 'Thickness must be less than half of width and height',
            ...
            this.materials = {
                ...
                'S235JR': { 
                    density: 7850, 
                    standard: 'EN 10025-2 / EN 10219-1',
                    group: 'structural'
                },
                'S275JR': { 
                    density: 7850, 
                ...
                'S460M': { 
                    density: 7850, 
                    standard: 'EN 10025-4 / EN 10219-1',
                    group: 'structural'
                },
                'S275J0H': { 
                    density: 7850, 
                ...
                '1.4301': { 
                    density: 7900, 
                    standard: 'EN 10088-2 / EN 10296-2',
                    group: 'stainless',
                    aisi: '304'
                },
                '1.4401': { 
            ...
            return `
                <div class="form-row">
                    <div class="form-group">
                        <label for="kp_en">${this.getText('en_label')}</label>
                        <input type="number" id="kp_en" min="0.1" step="0.1" placeholder="${lang === 'tr' ? 'En değeri' : 'Width value'}">
                    </div>
                    <div class="form-group">
                        <label for="kp_yukseklik">${this.getText('yukseklik_label')}</label>
                        <input type="number" id="kp_yukseklik" min="0.1" step="0.1" placeholder="${lang === 'tr' ? 'Yükseklik değeri' : 'Height value'}">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="kp_kalinlik">${this.getText('kalinlik_label')}</label>
                        <input type="number" id="kp_kalinlik" min="0.1" step="0.1" placeholder="${lang === 'tr' ? 'Kalınlık' : 'Thickness'}">
                    </div>
                    <div class="form-group">
                        <label for="kp_boy">${this.getText('boy_label')}</label>
                        <input type="number" id="kp_boy" min="0.1" step="0.1" placeholder="${lang === 'tr' ? 'Boy değeri' : 'Length value'}">
                    </div>
                </div>
            `;
        }
        ...
        calculate(formData) {
            const en = parseFloat(formData.kp_en) || 0;
            const yukseklik = parseFloat(formData.kp_yukseklik) || 0;
            const kalinlik = parseFloat(formData.kp_kalinlik) || 0;
            const boy = parseFloat(formData.kp_boy) || 0;
            const adet = parseFloat(formData.adet) || 1;
            const malzemeCinsi = formData.malzemeCinsi;
            
            if (en <= 0 || yukseklik <= 0 || kalinlik <= 0 || boy <= 0) {
                return null;
            }
            
            const density = this.densities[malzemeCinsi] || 7850;
            
            // Kutu profil kesit alanı formülü: A = 2 × t × (a + b - 2t)
            // Burada a = en, b = yükseklik, t = kalınlık (hepsi mm cinsinden)
            const kesitAlaniMM2 = 2 * kalinlik * (en + yukseklik - 2 * kalinlik);
        ...
        validate(formData) {
            ...
            const en = formData.kp_en;
            const yukseklik = formData.kp_yukseklik;
            const kalinlik = formData.kp_kalinlik;
            const boy = formData.kp_boy;
            ...
            if (yukseklik === undefined || yukseklik === null || yukseklik === '') {
                return {
                    isValid: false,
                    message: this.getText('validation_error')
                };
            ...
            if (isNaN(enNum) || isNaN(yukseklikNum) || isNaN(kalinlikNum) || isNaN(boyNum)) {
                return {
                    isValid: false,
                    message: this.getText('validation_error')
                };
            ...
            if (enNum <= 0 || yukseklikNum <= 0 || kalinlikNum <= 0 || boyNum <= 0) {
                return {
                    isValid: false,
            ...
            // Kalınlık kontrolü - kalınlık en ve yüksekliğin yarısından küçük olmalı
            if (kalinlikNum >= enNum / 2 || kalinlikNum >= yukseklikNum / 2) {
                return {
                    isValid: false,
        ...
        formatDimensions(formData) {
            ...
            const yukseklikStr = yukseklik % 1 === 0 ? yukseklik.toString() : yukseklik.toFixed(1);
            const kalinlikStr = kalinlik % 1 === 0 ? kalinlik.toString() : kalinlik.toFixed(1);
            const boyStr = boy % 1 === 0 ? boy.toString() : boy.toFixed(1);
            
            return `□${enStr} x ${yukseklikStr} x ${kalinlikStr} x ${boyStr}mm`;
        }
        ...
        formatRow(formData, calculation) {
            ...
            // Malzeme türünü override et (her zaman Kutu Profil)
            ...
            baseRow.metadata = {
                ...
                kutuProfil: {
                    en: formData.kp_en,
                    yukseklik: formData.kp_yukseklik,
                    kalinlik: formData.kp_kalinlik,
                    boy: formData.kp_boy,
        ...
        fillFormFromRow(rowData) {
            ...
            const metadata = rowData.metadata?.kutuProfil;
            if (metadata) {
                // Metadata'dan doldur
                setTimeout(() => {
                    const enElement = document.getElementById('kp_en');
                    const yukseklikElement = document.getElementById('kp_yukseklik');
                    const kalinlikElement = document.getElementById('kp_kalinlik');
                    const boyElement = document.getElementById('kp_boy');
                    
                    if (enElement) enElement.value = metadata.en;
                    if (yukseklikElement) yukseklikElement.value = metadata.yukseklik;
                ...
                setTimeout(() => {
                    ['kp_en', 'kp_yukseklik', 'kp_kalinlik', 'kp_boy'].forEach(fieldId => {
    ...
    const kutuProfilMaterial = new KutuProfilMaterial();
    kutuProfilMaterial.register();
    
    // Override getGrades fonksiyonunu özelleştirilmiş versiyonla
    const originalGetGrades = kutuProfilMaterial.getGrades.bind(kutuProfilMaterial);
    kutuProfilMaterial.getGrades = function() {
    ...
    console.log('Kutu Profil modülü v1.0.0 yüklendi');

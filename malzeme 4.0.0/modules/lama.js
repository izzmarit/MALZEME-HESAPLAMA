 ...
 * LAMA Malzeme Modülü
 * Versiyon: 1.0.0
 * Düz lama (flat bar) hesaplama modülü - EN standartları ile
 */
...
(function(window) {
    ...
    class LamaMaterial extends window.BaseMaterial {
            ...
            return `
                <div class="form-row">
                    <div class="form-group">
                        <label for="lm_en">${this.getText('en_label')}</label>
                        <input type="number" id="lm_en" min="0.1" step="0.1" placeholder="${lang === 'tr' ? 'En değeri' : 'Width value'}">
                    </div>
                    <div class="form-group">
                        <label for="lm_kalinlik">${this.getText('kalinlik_label')}</label>
                        <input type="number" id="lm_kalinlik" min="0.1" step="0.1" placeholder="${lang === 'tr' ? 'Kalınlık değeri' : 'Thickness value'}">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="lm_boy">${this.getText('boy_label')}</label>
                        <input type="number" id="lm_boy" min="0.1" step="0.1" placeholder="${lang === 'tr' ? 'Boy değeri' : 'Length value'}">
                    </div>
                </div>
            `;
        }
        ...
        calculate(formData) {
            const en = parseFloat(formData.lm_en) || 0;
            const kalinlik = parseFloat(formData.lm_kalinlik) || 0;
            const boy = parseFloat(formData.lm_boy) || 0;
            const adet = parseFloat(formData.adet) || 1;
            const malzemeCinsi = formData.malzemeCinsi;
            
            if (en <= 0 || kalinlik <= 0 || boy <= 0) {
                return null;
            }
            ...
            const hacimM3 = (en * kalinlik * boy) / 1000000000;
        ...
        validate(formData) {
            ...
            const en = formData.lm_en;
            const kalinlik = formData.lm_kalinlik;
            const boy = formData.lm_boy;
            ...
            if (isNaN(enNum) || isNaN(kalinlikNum) || isNaN(boyNum)) {
                return {
                    isValid: false,
                    message: this.getText('validation_error')
                };
            ...
            if (enNum <= 0 || kalinlikNum <= 0 || boyNum <= 0) {
                return {
                    isValid: false,
        ...
        formatDimensions(formData) {
            ...
            return `${enStr} x ${kalinlikStr} x ${boyStr}mm`;
        }
        ...
        formatRow(formData, calculation) {
            ...
            // Malzeme türünü override et (her zaman Lama)
        ...
        fillFormFromRow(rowData) {
            ...
            const metadata = rowData.metadata?.lama;
            if (metadata) {
                // Metadata'dan doldur
                setTimeout(() => {
                    const enElement = document.getElementById('lm_en');
                    const kalinlikElement = document.getElementById('lm_kalinlik');
                    const boyElement = document.getElementById('lm_boy');
                ...
                setTimeout(() => {
                    ['lm_en', 'lm_kalinlik', 'lm_boy'].forEach(fieldId => {
    ...
    const originalGetGrades = lamaMaterial.getGrades.bind(lamaMaterial);
    lamaMaterial.getGrades = function() {
    ...
    console.log('Lama modülü v1.0.0 yüklendi');

    ...
    const UIManager = {
        ...
        async exportFilteredData(filteredData, filterName) {
            ...
            try {
                ...
                // Seçilen modül isimlerini topla
                const selectedModuleNames = [];
                const uniqueTypes = new Set();
                
                filteredData.forEach(row => {
                    if (row.originalType && !uniqueTypes.has(row.originalType)) {
                        uniqueTypes.add(row.originalType);
                        
                        if (window.MaterialRegistry.has(row.originalType)) {
                            const MaterialClass = window.MaterialRegistry.get(row.originalType);
                            const instance = new MaterialClass();
                            selectedModuleNames.push(instance.getDisplayName());
                        }
                    }
                });
                
                // Filtreleme bilgisi - SİPARİŞ VE REVİZYON YOK!
                window._filterExportInfo = {
                    isFiltered: true,
                    filterName: filterName,
                    filterDate: new Date().toLocaleDateString('tr-TR'),
                    totalRecords: originalTableData.length,
                    filteredRecords: filteredData.length,
                    filterNames: selectedModuleNames
                };
                
                await window.ExcelManager.exportToExcel();
                
                delete window._filterExportInfo;

(function(window) {
    ...
    const TableManager = {
        ...
        loadTableData(data) {
            ...
            // P.No counter'ı ayarla
            let maxPNo = 0;
            data.forEach(row => {
                const pNoData = this.parsePartNumber(row.pNo);
                if (pNoData.main > maxPNo) {
                    maxPNo = pNoData.main;
                }
            });
            this.pNoCounter = maxPNo + 1;
            
            // Alt satır bilgilerini doğrula ve eksikleri tamamla
            this.validateSubRows();
        ...
        validateSubRows() {
            this.tableData.forEach((row, index) => {
                ...
                // isSubRow bilgisi yoksa P.No'dan belirle
                if (row.isSubRow === undefined) {
                    row.isSubRow = pNoData.isSubRow;
                }
                
                // parentPNo bilgisi yoksa P.No'dan belirle
                if (row.parentPNo === undefined || row.parentPNo === null) {
                    row.parentPNo = pNoData.main;
                }
            });
        },

...
(function(window) {
    ...
    class SockoletWeldoletMaterial extends window.BaseMaterial {
        ...
        formatDimensions(formData) {
            ...
            if (!fittingTipi || !headerNPS || !branchNPS) {
                return '-';
            }
            ...
            const fittingName = this.getText(fittingTipi).split(' ')[0];
            
            return `${fittingName} ${headerNPS}"x${branchNPS}" (${headerOD}x${branchOD}mm), Sch.${headerSchedule}x${branchSchedule}, Class ${classRating}, ${aci}°`;
        }
    ...
    console.log('Sockolet/Weldolet modülü v1.0.0 yüklendi (ASME B16.11, MSS SP-97)');

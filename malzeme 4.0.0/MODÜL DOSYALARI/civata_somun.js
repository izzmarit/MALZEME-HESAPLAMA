...
(function(window) {
    ...
    class CivataSomunMaterial extends window.BaseMaterial {
        ...
        setupEventHandlers() {
            ...
            window.CivataSomunHandlers = {
                ...
                onDiameterChange: function() {
                    ...
                    const currentBoy = boySelect.value;
                    ...
                    lengths.forEach(length => {
                        ...
                        // Eğer önceki değer mevcut listede varsa tekrar seç
                        if (length == currentBoy) {
                            foundCurrent = true;
                        }
                    });
                    
                    // Önceki değer hala geçerliyse geri yükle
                    if (foundCurrent && currentBoy) {
                        boySelect.value = currentBoy;
                    }
                }
            };
        }
        ...
        validate(formData) {
            ...
            // Undefined veya boş string kontrolü - dikkatli ve açık
            if (urunTipi === undefined || urunTipi === null || urunTipi === '') {
                return {
                    isValid: false,
            ...
            if (altTip === undefined || altTip === null || altTip === '') {
                return {
                    isValid: false,
                    message: this.getText('validation_error')
                };
            ...
            // Eğer ürün tipi civata ise boy kontrolü yap
            if (urunTipi === 'civata') {
                const boy = formData.cs_boy;
        ...
        getDisplayTypeFromRow(rowData) {
            // Cıvata/Somun için özel dil değişikliği yönetimi
            const metadata = rowData.metadata?.civataSomun;
            if (metadata && metadata.altTipKodu) {
                return this.getSubTypeName(metadata.altTipKodu);
            }
            // Fallback

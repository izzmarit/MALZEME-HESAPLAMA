...
(function(window) {
    ...
    class SacPlakaMaterial extends window.BaseMaterial {
        ...
        validate(formData) {
            const enInput = document.getElementById('sp_en');
            const boyInput = document.getElementById('sp_boy');
            const kalinlikInput = document.getElementById('sp_kalinlik');
            
            if (!enInput || !boyInput || !kalinlikInput) {
                return {
                    isValid: false,
            ...
            const boyValue = boyInput.value.trim();
            const kalinlikValue = kalinlikInput.value.trim();
            
            if (enValue === '' || boyValue === '' || kalinlikValue === '') {
                return {
                    isValid: false,
                    message: this.getText('validation_error')
                };
            ...
            const en = parseFloat(enValue);
            const boy = parseFloat(boyValue);
            const kalinlik = parseFloat(kalinlikValue);

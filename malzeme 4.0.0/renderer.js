...
const ApplicationController = {
    ...
    setupIPCListeners() {
        ...
        ipcRenderer.on('open-materials-info', () => {
            window.UIManager.showMaterialsInfo();
        });
    },
    ...
    setupKeyboardShortcuts() {
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Escape') {
                ...
                if (window.TableManager.editingIndex !== null) {
                    window.TableManager.cancelEdit();
                } else {
                    this.clearForm();
                }
            }
            ...
            if (input.id) {
                ...
                if (input.type === 'number') {
                    if (rawValue !== '' && rawValue !== null && rawValue !== undefined) {
                        ...
                        formData[input.id] = isNaN(numValue) ? 0 : numValue;
                    } else {
                        formData[input.id] = 0;
                    }
                } else if (input.type === 'text' || input.tagName === 'SELECT') {
                    formData[input.id] = rawValue || '';
                }
            }
        });

    ...
    const ExcelManager = {
        ...
        saveCustomMaterialsToHiddenSheet: function(workbook) {
            try {
                ...
                // Versiyon bilgisi
                metaSheet.addRow({ key: 'version', value: '3.0.0' });
                metaSheet.addRow({ key: 'created', value: new Date().toISOString() });
                
                // Özel malzeme şablonlarını kaydet
                const ozelMalzemeler = localStorage.getItem('ozelMalzemeler');
                if (ozelMalzemeler) {
                    metaSheet.addRow({ key: 'ozelMalzemeler', value: ozelMalzemeler });
                }
                
                // Özel flanş şablonlarını kaydet
                const ozelFlansTemplates = localStorage.getItem('ozelFlansTemplates');
                if (ozelFlansTemplates) {
                    metaSheet.addRow({ key: 'ozelFlansTemplates', value: ozelFlansTemplates });
                }
        ...
        loadCustomMaterialsFromHiddenSheet: function(workbook) {
            try {
                ...
                metaSheet.eachRow((row, rowNumber) => {
                    if (rowNumber === 1) return; // Başlık atla
                    
                    const key = row.getCell(1).value;
                    const value = row.getCell(2).value;
                    
                    if (key && value) {
                        if (key === 'ozelMalzemeler' || key === 'ozelFlansTemplates') {
                            try {
        ...
        setupColumnWidths: function(worksheet) {
            ...
            // *** YENİ: Metadata sütunu ***
            worksheet.getColumn(11).width = 5;  // Minimal genişlik çünkü gizli
        ...
        addDataRow: function(dataRow, data, fillStyle) {
            ...
            // *** YENİ: METADATA SÜTUNU EKLE (Sütun 11 - GİZLİ) ***
            ...
            const metadata = {
                ...
                isOzelFlans: data.isOzelFlans || false,
                formData: data.formData || {},
                // Malzeme tipine özel detaylar
                boruDetay: data.boruDetay || null,
                flansDetay: data.flansDetay || null,
                dirsekDetay: data.dirsekDetay || null,
                ozelMalzemeDetay: data.ozelMalzemeDetay || null,
                kosebentDetay: data.kosebentDetay || null,  // ✅ EKLENDI
                kutuDetay: data.kutuDetay || null,          // ✅ EKLENDI
                lamaDetay: data.lamaDetay || null,          // ✅ EKLENDI
                milDetay: data.milDetay || null,            // ✅ EKLENDI
                sacDetay: data.sacDetay || null,            // ✅ EKLENDI
                profilDetay: data.profilDetay || null,      // ✅ EKLENDI
                vanaDetay: data.vanaDetay || null,          // ✅ YENİ EKLENEN
                borulamaDetay: data.borulamaDetay || null,  // ✅ YENİ EKLENEN
                profilTipi: data.profilTipi || null,
                flansTipi: data.flansTipi || null,
                izgaraTipi: data.izgaraTipi || null,
                izgaraKategori: data.izgaraKategori || null
            };
        ...
        importFromExcel: async function() {
            try {
                ...
                // GİZLİ SAYFADAN METAVERİLERİ YÜKLE
            ...
            while (currentRow <= maxRows) {
                ...
                try {
                    ...
                    if (metadataValue) {
                        ...
                        rowData.isOzelFlans = metadata.isOzelFlans || false;
                        rowData.formData = metadata.formData || {};
                        
                        // Malzeme tipine özel detaylar
                        if (metadata.boruDetay) rowData.boruDetay = metadata.boruDetay;
                        if (metadata.flansDetay) rowData.flansDetay = metadata.flansDetay;
                        if (metadata.dirsekDetay) rowData.dirsekDetay = metadata.dirsekDetay;
                        if (metadata.ozelMalzemeDetay) rowData.ozelMalzemeDetay = metadata.ozelMalzemeDetay;
                        if (metadata.kosebentDetay) rowData.kosebentDetay = metadata.kosebentDetay;
                        if (metadata.kutuDetay) rowData.kutuDetay = metadata.kutuDetay;
                        if (metadata.lamaDetay) rowData.lamaDetay = metadata.lamaDetay;
                        if (metadata.milDetay) rowData.milDetay = metadata.milDetay;
                        if (metadata.sacDetay) rowData.sacDetay = metadata.sacDetay;
                        if (metadata.profilDetay) rowData.profilDetay = metadata.profilDetay;
                        if (metadata.vanaDetay) rowData.vanaDetay = metadata.vanaDetay;
                        
                        // BORULAMA DETAYI İÇİN KRİTİK EKLEME
                        if (metadata.borulamaDetay) {
                            rowData.borulamaDetay = metadata.borulamaDetay;
                            
                            // Eğer borulama ekipmanıysa ve tip bilgisi varsa, malzeme türünü güncelle
                            if (metadata.originalType === 'borulamaEkipmanlari' && metadata.borulamaDetay.tip) {
                                if (window.MaterialRegistry && window.MaterialRegistry.has('borulamaEkipmanlari')) {
                                    const MaterialClass = window.MaterialRegistry.get('borulamaEkipmanlari');
                                    const instance = new MaterialClass();
                                    rowData.malzemeTuru = instance.getText(metadata.borulamaDetay.tip);
                                }
                            }
                        }
                        
                        if (metadata.profilTipi) rowData.profilTipi = metadata.profilTipi;
                        if (metadata.flansTipi) rowData.flansTipi = metadata.flansTipi;
                        if (metadata.izgaraTipi) rowData.izgaraTipi = metadata.izgaraTipi;
                        if (metadata.izgaraKategori) rowData.izgaraKategori = metadata.izgaraKategori;
                        
                        console.log(`Satır ${currentRow}: Metadata başarıyla okundu`, metadata);
                    } else {
                        // Metadata yoksa eski yöntemle belirle
                        console.log(`Satır ${currentRow}: Metadata yok, eski yöntem kullanılıyor`);
                ...
                // Revizyon durumunu kontrol et (font rengi kontrolü yedek olarak)
                ...
                // Eğer originalType hala boşsa, eski yöntemle belirle
        ...
        determineMaterialType: function(rowData) {
            // *** YENİ: Metadata'dan gelen bilgi varsa öncelik ver ***
            if (rowData.originalType && rowData.originalType !== '') {
                console.log('originalType metadata\'dan geldi:', rowData.originalType);
                return; // Zaten belirlenmiş, devam etme
            }
            
            const malzemeTuruLower = rowData.malzemeTuru.toString().toLowerCase();
            const malzemeTuruOriginal = rowData.malzemeTuru.toString();
            
            // !!!! ÖNCELİK 1: Özel Flanş kontrolü - EN BAŞTA OLMALI !!!!
            // isOzelFlans marker'ı kontrolü
            if (rowData.isOzelFlans === true) {
                rowData.originalType = 'ozelFlans';
                return;
            }
            
            // flansDetay içinde disCap varsa ÖZEL FLANŞ (normal flanşlarda disCap yok)
            if (rowData.flansDetay && rowData.flansDetay.disCap !== undefined) {
                rowData.originalType = 'ozelFlans';
                return;
            }
            
            // !!!! ÖNCELİK 2: Izgara elemanı kontrolü !!!!
            ...
            // !!!! ÖNCELİK 3: Normal Flanş kontrolü - flansTipi field'ı !!!!
            // ANCAK disCap yoksa (özel flanş değilse)
            ...
            // ANCAK flansDetay'da disCap yoksa (özel flanş değilse)
            ...
            for (const tip of flansTipleri) {
                if (malzemeTuruLower.includes(tip)) {
                    // flansDetay içinde disCap YOKSA normal flanş
                    if (!rowData.flansDetay || rowData.flansDetay.disCap === undefined) {
                        rowData.originalType = 'flans';
                        return;
                    }
            ...
            // Vana kontrolü
            if (malzemeTuruLower.includes('vana') || malzemeTuruLower.includes('valve')) {
                rowData.originalType = 'vana';
                return;
            }

            // Borulama ekipmanları kontrolü - daha detaylı
            if (rowData.borulamaDetay && rowData.borulamaDetay.tip) {
                rowData.originalType = 'borulamaEkipmanlari';
                return;
            }

            // Malzeme türü adından borulama ekipmanı tespiti
            const borulamaEkipmanTipleri = {
                'Te (Eşit)': 'tee_equal',
                'Te (Redüksiyonlu)': 'tee_reducing',
                'Tee (Equal)': 'tee_equal',
                'Tee (Reducing)': 'tee_reducing',
                'Redüksiyon (Konsantrik)': 'reducer_concentric',
                'Redüksiyon (Eksantrik)': 'reducer_eccentric',
                'Reducer (Concentric)': 'reducer_concentric',
                'Reducer (Eccentric)': 'reducer_eccentric',
                'Y Tipi Süzgeç': 'strainer_y',
                'T Tipi Süzgeç': 'strainer_t',
                'Y-Type Strainer': 'strainer_y',
                'T-Type Strainer': 'strainer_t',
                'Körüklü Kompansatör': 'bellows_single',
                'Bellows Compensator': 'bellows_single',
                'Spiral Sargılı Conta': 'gasket_spiral',
                'Spiral Wound Gasket': 'gasket_spiral'
            };

            // Tam eşleşme kontrolü
            if (borulamaEkipmanTipleri[rowData.malzemeTuru]) {
                rowData.originalType = 'borulamaEkipmanlari';
                // Eğer borulamaDetay yoksa minimal bilgiyle oluştur
                if (!rowData.borulamaDetay) {
                    rowData.borulamaDetay = {
                        tip: borulamaEkipmanTipleri[rowData.malzemeTuru],
                        kategori: 'fittings' // Varsayılan kategori
                    };
                }
                return;
            }
            ...
            const borulamaAnahtarKelimeler = [
                'fitting', 'te ', 'tee', 'redüksiyon', 'reducer', 
                'dirsek', 'elbow', 'kaplin', 'coupling', 'nipel', 
                'kompansatör', 'compensator', 'süzgeç', 'strainer', 
                'conta', 'gasket', 'askı', 'hanger', 'manometre', 
                'gauge', 'termometre', 'weldolet', 'threadolet'
            ];

            for (const anahtar of borulamaAnahtarKelimeler) {
                if (malzemeTuruLower.includes(anahtar)) {
                    rowData.originalType = 'borulamaEkipmanlari';
                    return;
                }
            }

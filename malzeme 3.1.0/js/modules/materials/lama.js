...
(function(window) {
    ...
    class LamaMaterial extends window.BaseMaterial {
        ...
        fillSpecificFields(rowData) {
                // ÖNCELİK 0: Metadata'dan gelen lamaDetay
            if (rowData.lamaDetay) {
                console.log('Lama metadata lamaDetay bulundu:', rowData.lamaDetay);
                
                ['en', 'kalinlik', 'uzunluk'].forEach(field => {
                    const element = document.getElementById(field);
                    if (element && rowData.lamaDetay[field] !== undefined) {
                        element.value = rowData.lamaDetay[field];
                    }
                });
                return;
            }

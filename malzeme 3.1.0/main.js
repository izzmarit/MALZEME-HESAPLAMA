            ...
            submenu: [
                {
                    ...
                    click: () => {
                        mainWindow.webContents.send('add-module');
                    }
                },
                {
                    label: 'Modülleri Yönet',
                    click: () => {
                        mainWindow.webContents.send('manage-modules');
                    }
                },
                { type: 'separator' },
                {
                    label: 'Modül Güncelle',
                    click: () => {
                        mainWindow.webContents.send('update-modules');
                    }
                }
            ]
        },
...
ipcMain.handle('install-module', async (event, modulePath) => {
    try {
        const userDataPath = app.getPath('userData');
        const customModulesPath = path.join(userDataPath, 'custom_modules');
        
        // Klasör yoksa oluştur
        if (!fs.existsSync(customModulesPath)) {
            fs.mkdirSync(customModulesPath, { recursive: true });
        }
        
        const moduleContent = fs.readFileSync(modulePath, 'utf8');
        const moduleName = path.basename(modulePath);
        const targetPath = path.join(customModulesPath, moduleName);
        
        fs.writeFileSync(targetPath, moduleContent);
        
        return { success: true, moduleName, targetPath };
    } catch (error) {
        return { success: false, error: error.message };
    }
});

ipcMain.handle('get-custom-modules', async () => {
    try {
        const userDataPath = app.getPath('userData');
        const customModulesPath = path.join(userDataPath, 'custom_modules');
        ...
        const files = fs.readdirSync(customModulesPath)
            .filter(file => file.endsWith('.js'));
        
        return { success: true, modules: files };
    } catch (error) {
        return { success: false, error: error.message };
    }
});
...
ipcMain.handle('select-module-file', async () => {
    const result = await dialog.showOpenDialog(mainWindow, {
        ...
        filters: [
            ...
            { name: 'Modül Paketleri', extensions: ['zip'] }
        ],
        properties: ['openFile']
    });
...
ipcMain.handle('remove-module', async (event, moduleName) => {
    try {
        const userDataPath = app.getPath('userData');
        const customModulesPath = path.join(userDataPath, 'custom_modules');
        ...
        if (fs.existsSync(modulePath)) {
            ...
            return { success: false, error: 'Modül dosyası bulunamadı' };
        }
    } catch (error) {
        return { success: false, error: error.message };
    }

(function(window) {
    ...
    const ExcelManager = {
        ...
        addProjectInfo(worksheet, projectInfo, texts) {
            ...
            const safeMergeCells = (ws, range) => {
                try {
                    // Hücre zaten birleştirilmiş mi kontrol et
                    const model = ws.getCell(range.split(':')[0])._mergeCount;
                    if (!model || model === 0) {
                        ws.mergeCells(range);
                    }
                } catch (error) {
                    ...
                    try {
                        ...
                        console.warn(`Hücre birleştirme atlandı: ${range}`);
                    }
                }
            };
            ...
            // G1 hücresi - Proje Adı
            const projeAdiCell = worksheet.getCell('G1');
            const projeAdi = projectInfo.projeAdi || '';
            projeAdiCell.value = projeAdi;
            
            let projeAdiFontSize = 16;
            const projeAdiLength = projeAdi.length;
            
            if (projeAdiLength > 50) {
                projeAdiFontSize = 10;
            } else if (projeAdiLength > 40) {
                projeAdiFontSize = 11;
            } else if (projeAdiLength > 30) {
                projeAdiFontSize = 12;
            } else if (projeAdiLength > 20) {
                projeAdiFontSize = 14;
            }
            
            projeAdiCell.font = { 
                name: 'Calibri', 
                size: projeAdiFontSize, 
                bold: true 
            };
            
            projeAdiCell.alignment = { 
                horizontal: 'center', 
                vertical: 'center',
                wrapText: true
            };
            ...
            // Filtreleme bilgisi varsa ekle
            if (window._filterExportInfo && window._filterExportInfo.isFiltered) {
                const filterInfo = window._filterExportInfo;
                
                // J3:J4 birleştirme - güvenli yöntem
                safeMergeCells(worksheet, 'J3:J4');
                
                const filterCell = worksheet.getCell('J3');
                filterCell.value = `FİLTRELENMİŞ LİSTE\n${filterInfo.filterName}`;
                filterCell.font = { 
                    name: 'Calibri', 
                    size: 9, 
                    bold: true,
                ...
                // Özet bilgisi
                const filterDetailCell = worksheet.getCell('J6');
                filterDetailCell.value = `${filterInfo.filteredRecords} / ${filterInfo.totalRecords} kayıt`;
                filterDetailCell.font = { 
                    name: 'Calibri', 
                    size: 8,
                    italic: true,
                    color: { argb: 'FFFF0000' }
                };
                filterDetailCell.alignment = { 
                    horizontal: 'center', 
                    vertical: 'center'
                };
            }
        ...
        addDataRow(dataRow, data, fillStyle) {
            ...
            // Debug için konsola yazdır (isteğe bağlı - üretimde kaldırılabilir)
            if (maxLineCount > 1) {
                console.log('Satır yüksekliği hesaplama:', {
                    'Malzeme Türü': data.malzemeTuru,
                    'Satır sayıları': lineCountPerColumn,
                    'Max satır': maxLineCount
                });
            }
            
            // Satır yüksekliğini ayarla
            // Her satır için 15 birim, minimum 18.75, maksimum 90

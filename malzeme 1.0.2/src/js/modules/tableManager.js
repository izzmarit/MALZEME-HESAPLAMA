...
(function(window) {
    ...
    addRow: function() {
        ...
        // Özel malzeme türlerini de formatlara ekle
        const allTypes = MaterialData.getAllMaterialTypes();
        Object.keys(allTypes).forEach(kod => {
            if (!malzemeTuruFormatlari[kod]) {
                malzemeTuruFormatlari[kod] = allTypes[kod].ad;
            }
        });
        
        // Özel malzeme türleri kontrolü (birim ağırlık tipi dahil)
        const typeData = MaterialData.getAllMaterialTypes()[tur];
        if (typeData && typeData.custom) {
            malzemeTuruGosterim = typeData.ad;
            
            if (typeData.hesaplamaTipi === 'birimAgirlik') {
                // Birim ağırlık tipi için özel işlem
                const customBirimAgirlik = document.getElementById('customBirimAgirlik');
                const customOlculer = document.getElementById('customOlculer');
                const customMalzemeCinsi = document.getElementById('customMalzemeCinsi');
                const customStandart = document.getElementById('customStandart');
                
                birimAgirlik = parseFloat(customBirimAgirlik ? customBirimAgirlik.value : 0) || 0;
                olculer = customOlculer ? customOlculer.value : '-';
                malzemeCinsi = customMalzemeCinsi ? customMalzemeCinsi.value : '-';
                enNormu = customStandart ? customStandart.value : '-';
                ...
                // Normal özel malzeme türleri için
        ...
        if (tur !== 'ozelMalzeme' && tur !== 'izgara' && 
            !(typeData && typeData.custom && typeData.hesaplamaTipi === 'birimAgirlik') && 
            birimAgirlik === 0) {
            this.showNotification('Lütfen önce hesaplama yapın!', 'warning');
            return false;
        }
        ...
        const rowData = {
            ...
            customTypeData: typeData && typeData.custom ? typeData : null
        };
    ...
    fillDimensionsFromRow: function(row) {
        ...
        if (typeData && typeData.custom) {
            if (typeData.hesaplamaTipi === 'birimAgirlik') {
                // Birim ağırlık tipi için
                const customBirimAgirlik = document.getElementById('customBirimAgirlik');
                const customOlculer = document.getElementById('customOlculer');
                const customMalzemeCinsi = document.getElementById('customMalzemeCinsi');
                const customStandart = document.getElementById('customStandart');
                
                if (customBirimAgirlik) customBirimAgirlik.value = row.birimAgirlik;
                if (customOlculer) customOlculer.value = row.olculer === '-' ? '' : row.olculer;
                if (customMalzemeCinsi) customMalzemeCinsi.value = row.malzemeCinsi === '-' ? '' : row.malzemeCinsi;
                if (customStandart) customStandart.value = row.enNormu === '-' ? '' : row.enNormu;
        ...
        switch(tur) {
            ...
            const typeData = MaterialData.getAllMaterialTypes()[tur];
            if (typeData && typeData.custom && typeData.olcuAlanlari) {
                const degerler = olculer.match(/[\d.]+/g);
                if (degerler && degerler.length >= typeData.olcuAlanlari.length) {
                    typeData.olcuAlanlari.forEach((alan, index) => {
    ...
    updateRow: function() {
        ...
        // Özel malzeme türlerini kontrol et (BİRİM AĞIRLIK DAHİL)
        const typeData = MaterialData.getAllMaterialTypes()[tur];
        if (typeData && typeData.custom && typeData.hesaplamaTipi === 'birimAgirlik') {
            // Birim ağırlık tipi özel malzeme için
            const customBirimAgirlik = document.getElementById('customBirimAgirlik');
            const customOlculer = document.getElementById('customOlculer');
            const customMalzemeCinsi = document.getElementById('customMalzemeCinsi');
            const customStandart = document.getElementById('customStandart');
            ...
            const birimAgirlik = parseFloat(customBirimAgirlik ? customBirimAgirlik.value : 0) || 0;
            
            if (birimAgirlik === 0) {
                UIManager.showNotification('Lütfen birim ağırlık değeri girin!', 'warning');
                return;
            }
        ...
        // Özel malzeme kontrolü
            ...
            // Normal malzemeler ve diğer özel tipler için hesapla
            if (!Calculator.calculate()) {
            ...
            if (tur === 'flans') {
                const flansTipi = document.getElementById('flansTipi')?.value;
                malzemeTuruGosterim = MaterialData.FLANS_TIPLERI[flansTipi];
            } else if (typeData && typeData.custom) {
                // Diğer özel malzeme türleri için
                malzemeTuruGosterim = typeData.ad;
            }

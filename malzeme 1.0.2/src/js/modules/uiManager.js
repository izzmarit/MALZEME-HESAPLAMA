...
(function(window) {
    ...
    openModal: function(content, title = 'Ayarlar') {
        ...
        // Sınıf ile yönet, inline style kullanma
        modal.classList.remove('hidden');
        modal.style.display = ''; // Inline style'ı temizle
        
        // Focus yönetimi
        setTimeout(() => {
            const firstInput = modalBody.querySelector('input:not([type="checkbox"]):not([readonly]), select');
            if (firstInput) {
                firstInput.focus();
            }
        }, 50);
    ...
    closeModal: function() {
        ...
        if (typeof window.cleanupCheckboxListeners === 'function') {
            window.cleanupCheckboxListeners();
        }
        
        // Global değişkenleri sıfırla
        if (window.selectedFieldOrder) {
        ...
        // Focus'u geri ver - ÖNEMLİ
        document.body.focus();
        setTimeout(() => {
            const activeInput = document.querySelector('#malzemeTuru, input[type="number"]:not([readonly]), input[type="text"]:not([readonly])');
            if (activeInput) {
                activeInput.focus();
            }
        }, 0);

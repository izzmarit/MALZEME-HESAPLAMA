...
(function(window) {
    ...
    calculateHacimTabanliMalzeme: function(malzemeData) {
        ...
        malzemeData.olcuAlanlari.forEach(alan => {
            ...
            if (input) {
                olcuDegerleri[alan.id] = parseFloat(input.value) || 0;
            }
        });
        ...
        let hacim = 0;
        try {
            // Güvenli eval için Function constructor kullan
            const formula = new Function(...Object.keys(olcuDegerleri), `return ${malzemeData.hesaplamaFormulu}`);
            hacim = formula(...Object.values(olcuDegerleri)) / 1000000000; // mm³ -> m³
        } catch (error) {
            console.error('Formül hesaplama hatası:', error);
            throw new Error('Hacim hesaplaması başarısız');
        }
        
        const agirlik = hacim * yogunluk;
        
        // Ölçüleri formatla
        const olculer = malzemeData.olcuAlanlari
            .map(alan => `${olcuDegerleri[alan.id]}${alan.birim}`)
            .join('x');

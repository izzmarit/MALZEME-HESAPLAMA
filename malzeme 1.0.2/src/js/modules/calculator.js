...
(function(window) {
    ...
    calculateCustomType: function(tur, yogunluk) {
        ...
        if (typeData.hesaplamaTipi === 'birimAgirlik') {
            ...
            const malzemeCinsiInput = document.getElementById('customMalzemeCinsi');
            const standartInput = document.getElementById('customStandart');
            
            const agirlik = parseFloat(birimAgirlikInput ? birimAgirlikInput.value : 0) || 0;
            const olculer = olculerInput ? olculerInput.value : '';
            const malzemeCinsi = malzemeCinsiInput ? malzemeCinsiInput.value : '';
            const standart = standartInput ? standartInput.value : '';
            
            // Malzeme cinsi ve standart bilgilerini currentCalculation'a sakla
            this.currentCalculation.customMalzemeCinsi = malzemeCinsi;
            this.currentCalculation.customStandart = standart;
            this.currentCalculation.customOlculer = olculer;
            this.currentCalculation.isCustomBirimAgirlik = true;
            this.currentCalculation.customTypeData = typeData;
            
            return { agirlik, olculer };
        }
        
        // Normal hesaplama için mevcut kod devam eder
        const olcuDegerleri = [];
        let olculerText = '';

...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.MaterialCalculator = {
            ...
            if (malzemeData.tip === 'hacimTabanli') {
                ...
                const olcuAlanlariHtml = malzemeData.olcuAlanlari.map(alan => `
                    <div class="form-group">
                        ...
                        <input type="number" id="${alan.id}" step="0.1" min="0" placeholder="0.0">
                    </div>
                `).join('');
                ...
                <div class="material-add-tabs">
                    ...
                    <div id="hacim-tab" class="tab-content">
                        ...
                        <div id="olcuAlanlariContainer">
                            <h5>Ölçü Alanları</h5>
                            <div id="olcuAlanlariList"></div>
                            <button onclick="MaterialCalculator.addOlcuAlani()" class="btn btn-add" style="margin-top: 10px;">
                                + Ölçü Alanı Ekle
                            </button>
                        </div>
                        ...
                        <div class="form-group" style="margin-top: 15px;">
                            ...
                            <input type="text" id="hacimFormulu" placeholder="Örn: kalinlik * en * boy">
                    ...
                    <div id="birim-tab" class="tab-content" style="display: none;">
                        ...
                        <div class="form-group">
                            <label>Malzeme Adı *</label>
                            <input type="text" id="birimMalzemeAdi" placeholder="Örn: Hazır Flanş">
                        </div>
                        
                        <div class="form-group">
                            <label>Birim Ağırlık (kg) *</label>
                            <input type="number" id="birimAgirlik" step="0.01" placeholder="0.00">
                        </div>
                        ...
                        <div style="margin-top: 20px;">
                            <button onclick="MaterialCalculator.saveBirimTabanliMalzeme()" class="btn btn-success">
                                Kaydet
                            </button>
                        </div>
                    </div>
        ...
        // Ölçü alanı ekleme
        olcuAlanlariSayac: 0,
        addOlcuAlani: function() {
            this.olcuAlanlariSayac++;
            const container = document.getElementById('olcuAlanlariList');
            ...
            div.innerHTML = `
                ...
                <button onclick="this.parentElement.remove()" style="padding: 5px 10px; background: #f56565; color: white; border: none; border-radius: 4px; cursor: pointer;">Sil</button>
            `;
        ...
        saveHacimTabanliMalzeme: function() {
            const malzemeAdi = document.getElementById('hacimMalzemeAdi').value;
            const formul = document.getElementById('hacimFormulu').value;
            
            if (!malzemeAdi || !formul) {
                UIManager.showNotification('Lütfen tüm zorunlu alanları doldurun', 'warning');
                return;
            }
            
            // Ölçü alanlarını topla
            const olcuAlanlari = [];
            document.querySelectorAll('.olcu-alani-item').forEach((item, index) => {
                const id = item.querySelector(`[id^="olcuId"]`).value;
                const label = item.querySelector(`[id^="olcuLabel"]`).value;
                const birim = item.querySelector(`[id^="olcuBirim"]`).value;
            ...
            const kod = MaterialData.ozelMalzemeTuruEkleHacimTabanli(malzemeAdi, olcuAlanlari, formul);
        ...
        saveBirimTabanliMalzeme: function() {
            const malzemeAdi = document.getElementById('birimMalzemeAdi').value;
            const birimAgirlik = document.getElementById('birimAgirlik').value;
            const olculer = document.getElementById('birimOlculer').value;
            const standart = document.getElementById('birimStandart').value;
            const malzemeCinsi = document.getElementById('birimMalzemeCinsi').value;
            
            if (!malzemeAdi || !birimAgirlik) {
                UIManager.showNotification('Lütfen zorunlu alanları doldurun', 'warning');
                return;
            }

...
// Dosyanın başına ekleyin - Event listener referanslarını saklamak için
window.modalEventListeners = {
    closeModal: null,
    deleteButtons: []
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Event listener'ları saklamak için global değişken
    window.currentCheckboxListeners = [];

    // Event listener temizleme fonksiyonu
    window.cleanupCheckboxListeners = function() {
        window.currentCheckboxListeners.forEach(({element, listener}) => {
    ...
    window.MaterialCalculator = {
        ...
        malzemeTuruDegisti: function() {
            ...
            // htmlContent değişkenini en başta tanımla
            let htmlContent = '';
            ...
            if (typeData && typeData.custom) {
                ...
                if (typeData.hesaplamaTipi === 'birimAgirlik') {
                    ...
                    const varsayilanlar = typeData.varsayilanDegerler || {};
                    ...
                    htmlContent = `
                        <div class="form-row">
                            <div class="form-group full-width">
                                ...
                                <input type="number" id="customBirimAgirlik" step="0.01" min="0" 
                                    value="${varsayilanlar.birimAgirlik || ''}" 
                                    placeholder="0.00">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group full-width">
                                ...
                                <input type="text" id="customOlculer" 
                                    value="${varsayilanlar.olculer || ''}" 
                                    placeholder="İsteğe bağlı ölçü bilgisi">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group full-width">
                                <label for="customMalzemeCinsi">
                                    <span class="label-icon">🔬</span> Malzeme Cinsi
                                </label>
                                <input type="text" id="customMalzemeCinsi" 
                                    value="${varsayilanlar.malzemeCinsi || ''}" 
                                    placeholder="İsteğe bağlı malzeme cinsi">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group full-width">
                                ...
                                <input type="text" id="customStandart" 
                                    value="${varsayilanlar.standart || ''}" 
                                    placeholder="İsteğe bağlı standart bilgisi">
                            </div>
                        </div>
                    `;
            ...
            if (['ipe', 'hea', 'heb', 'npu', 'npi'].includes(tur)) {
    ...
    // FOCUS YÖNETİMİ İÇİN GLOBAL EVENT HANDLER - EKLE
    document.addEventListener('click', (e) => {
        // Her tıklamada focus kontrolü yap
        setTimeout(() => {
            const focused = document.activeElement;
            if (!focused || focused === document.body || focused === document.documentElement) {
                const firstInput = document.querySelector('input:not([readonly]):not([type="hidden"]), select, textarea');
                if (firstInput) {
                    firstInput.focus();
                }
            }
    ...
    // Window focus eventi için listener
    ...
    // Başlangıç mesajı - FOCUS İÇİN ÖNEMLİ
    setTimeout(() => {
        UIManager.showNotification('Program başarıyla yüklendi', 'success');
        
        // İlk focus'u malzeme türüne ver
        const malzemeTuruSelect = document.getElementById('malzemeTuru');
        if (malzemeTuruSelect) {
            malzemeTuruSelect.focus();
        }
    }, 500);

    // Kayıtlı verileri yükle - ASENKRON İŞLEM TAMAMLANDIKTAN SONRA FOCUS
    loadSavedData().then(() => {
        // Veri yükleme tamamlandıktan sonra focus'u ayarla
        const malzemeTuruSelect = document.getElementById('malzemeTuru');
        if (malzemeTuruSelect) {
            malzemeTuruSelect.focus();
        }
...
function setupEventListeners() {
    ...
    if (closeModal) {
        if (window.modalEventListeners.closeModal) {
            closeModal.removeEventListener('click', window.modalEventListeners.closeModal);
        }
        
        // Yeni listener fonksiyonu
        window.modalEventListeners.closeModal = function() {
            const modal = document.getElementById('settingsModal');
            if (modal) {
                modal.classList.add('hidden');
                
                // Focus'u ana forma geri ver
                setTimeout(() => {
        ...
        closeModal.addEventListener('click', window.modalEventListeners.closeModal);
    }
}
...
function showMaterialTypesManager() {
    ...
    const typesList = Object.entries(types).map(([kod, data]) => {
        ...
        let olcuText = '';
        if (data.hesaplamaTipi === 'birimAgirlik') {
            olcuText = 'Birim Ağırlık Girişi';
        } else if (data.olcuAlanlari) {
    ...
    const content = `
        <div class="material-manager">
            <div class="manager-section">
                ...
                <div id="birimAgirlikFields" style="display: none;">
                    <div class="form-group">
                        <label>Varsayılan Birim Ağırlık (kg)</label>
                        <input type="number" id="defaultBirimAgirlik" step="0.01" placeholder="0.00">
                    </div>
                    <div class="form-group">
                        <label>Varsayılan Malzeme Cinsi</label>
                        <input type="text" id="defaultMalzemeCinsi" placeholder="Örn: S235JR">
                    </div>
                    <div class="form-group">
                        <label>Varsayılan Standart</label>
                        <input type="text" id="defaultStandart" placeholder="Örn: EN 10025-2">
                    </div>
                    <div class="form-group">
                        <label>Varsayılan Ölçüler</label>
                        <input type="text" id="defaultOlculer" placeholder="Örn: 100x50x10mm">
                    </div>
                </div>
...
window.setupOrderedCheckboxes = function() {
    ...
    window.cleanupCheckboxListeners();
    ...
    checkboxes.forEach(checkbox => {
        ...
        // window.currentCheckboxListeners kullanımına dikkat edin
        window.currentCheckboxListeners.push({element: checkbox, listener: listener});
    });
};
...
window.toggleOlcuAlanlar = function() {
    ...
    const birimAgirlikFields = document.getElementById('birimAgirlikFields');
    const unitSelect = document.getElementById('newTypeUnit');
    
    if (hesaplamaTipi === 'birimAgirlik') {
        if (olcuAlanlarGroup) olcuAlanlarGroup.style.display = 'none';
        
        // Birim ağırlık alanlarını oluştur veya göster
        if (birimAgirlikFields) {
            birimAgirlikFields.style.display = 'block';
        } else {
            // Eğer birimAgirlikFields yoksa dinamik olarak oluştur
            const managerSection = document.querySelector('.manager-section');
            if (managerSection) {
                const birimAgirlikDiv = document.createElement('div');
                birimAgirlikDiv.id = 'birimAgirlikFields';
                birimAgirlikDiv.innerHTML = `
                    <div class="form-group">
                        <label>Varsayılan Birim Ağırlık (kg)</label>
                        <input type="number" id="defaultBirimAgirlik" step="0.01" placeholder="0.00">
                    </div>
                ...
                `;
                // Form gruplarından sonra ekle
                const addButton = managerSection.querySelector('button[onclick="addNewMaterialType()"]');
                if (addButton) {
                    addButton.parentNode.insertBefore(birimAgirlikDiv, addButton);
                }
            }
        }
        ...
        if (olcuAlanlarGroup) olcuAlanlarGroup.style.display = 'block';
        if (birimAgirlikFields) birimAgirlikFields.style.display = 'none';
        if (unitSelect) {
...
window.addNewMaterialType = function() {
    ...
    if (!kod || !ad) {
        UIManager.showNotification('Lütfen kod ve ad alanlarını doldurun', 'warning');
        return;
    }
    ...
    if (hesaplamaTipi === 'birimAgirlik') {
        // Birim ağırlık için varsayılan değerleri al
        const defaultBirimAgirlik = document.getElementById('defaultBirimAgirlik').value;
        const defaultMalzemeCinsi = document.getElementById('defaultMalzemeCinsi').value;
        const defaultStandart = document.getElementById('defaultStandart').value;
        const defaultOlculer = document.getElementById('defaultOlculer').value;
        
        typeData.varsayilanDegerler = {
            birimAgirlik: parseFloat(defaultBirimAgirlik) || 0,
            malzemeCinsi: defaultMalzemeCinsi || '',
            standart: defaultStandart || '',
            olculer: defaultOlculer || ''
        };
        
        typeData.olcuAlanlari = [];
    } else {
        // Normal hesaplama için ölçü alanlarını al
        if (!window.selectedFieldOrder || window.selectedFieldOrder.length === 0) {
            UIManager.showNotification('Lütfen en az bir ölçü alanı seçin', 'warning');
            return;
        }
        typeData.olcuAlanlari = window.selectedFieldOrder;
    }
    
    // MaterialData modülüne ekle
    const customTypes = MaterialData.getCustomMaterialTypes();
    
    if (MaterialData.MALZEME_TURLERI[kod] || customTypes[kod]) {
        UIManager.showNotification('Bu kod zaten kullanımda', 'error');
        return;
    }
    
    customTypes[kod] = typeData;
    MaterialData.saveCustomMaterialTypes(customTypes);
    MaterialData.MALZEME_TURLERI[kod] = customTypes[kod];
    
    UIManager.showNotification('Malzeme türü başarıyla eklendi', 'success');
    
    // Modal'ı kapat ve yenile
    setTimeout(() => {
...
window.deleteMaterialType = function(kod) {
    ...
    const userConfirm = confirm(`"${kod}" malzeme türünü silmek istediğinizden emin misiniz?`);
    
    // Confirm dialog'dan sonra focus'u düzelt
    setTimeout(() => {
        ...
        if (userConfirm) {
            if (MaterialData.deleteMaterialType(kod)) {
                ...
                // Tabloyu güncelle - innerHTML kullanmadan
                updateMaterialTypesTable();
                
                // Focus'u input'a ver
                const newTypeCode = document.getElementById('newTypeCode');
                if (newTypeCode) {
                    newTypeCode.focus();
                }
            }
...
// Yeni fonksiyon: Modal içeriğini güncelleme
window.refreshMaterialTypesManager = function() {
    const modalBody = document.getElementById('settingsBody');
    const modalHeader = document.querySelector('.modal-header h2');
    
    if (modalBody && modalHeader) {
        // Event listener'ları temizle
        cleanupCheckboxListeners();
        window.selectedFieldOrder = [];
        
        // Sadece içeriği güncelle, modal'ı kapatma
        const types = MaterialData.getAllMaterialTypes();
        const content = generateMaterialTypesContent(types);
        
        modalBody.innerHTML = content;
        modalHeader.textContent = 'Malzeme Türleri Yönetimi';
        ...
        setTimeout(() => {
            const firstInput = modalBody.querySelector('input[type="text"]');
            if (firstInput) {
                firstInput.focus();
            }
        }, 100);
...
window.deleteMaterialGrade = function(kod) {
    ...
    const userConfirm = confirm(`"${kod}" malzeme cinsini silmek istediğinizden emin misiniz?`);
    ...
    setTimeout(() => {
        ...
        if (userConfirm) {
            if (MaterialData.deleteMaterialGrade(kod)) {
                ...
                // Tabloyu güncelle - innerHTML kullanmadan
                updateMaterialGradesTable();
                
                // Focus'u input'a ver
                const newGradeCode = document.getElementById('newGradeCode');
                if (newGradeCode) {
                    newGradeCode.focus();
                }
            }
...
// Yeni yardımcı fonksiyonlar ekleyin
window.updateMaterialTypesTable = function() {
    const types = MaterialData.getAllMaterialTypes();
    ...
    Object.entries(types).forEach(([kod, data]) => {
        ...
        tdKod.textContent = kod;
        tr.appendChild(tdKod);
        
        const tdAd = document.createElement('td');
        tdAd.textContent = data.ad;
        tr.appendChild(tdAd);
        
        const tdOlcu = document.createElement('td');
        if (data.hesaplamaTipi === 'birimAgirlik') {
            tdOlcu.textContent = 'Birim Ağırlık Girişi';
        } else if (data.olcuAlanlari) {
            tdOlcu.textContent = data.olcuAlanlari.map(alan => 
                typeof alan === 'object' ? alan.ad : alan
            ).join(', ');
        } else {
            tdOlcu.textContent = data.tip || 'Standart';
        }
        tr.appendChild(tdOlcu);
        
        const tdHesaplama = document.createElement('td');
        tdHesaplama.textContent = data.hesaplamaTipi || 'Varsayılan';
        tr.appendChild(tdHesaplama);
        ...
        if (data.custom) {
            ...
            deleteBtn.addEventListener('click', function(e) {
                e.stopPropagation();
                window.deleteMaterialType(kod);
            });
            tdIslem.appendChild(deleteBtn);
        } else {
            const span = document.createElement('span');
            span.className = 'text-muted';
            span.textContent = 'Sistem';
            tdIslem.appendChild(span);
        }
...
window.updateMaterialGradesTable = function() {
    const grades = MaterialData.getAllMaterialGrades();
    const tableBody = document.querySelector('.manager-table tbody');
    ...
    grades.forEach(grade => {
        ...
        const tdKod = document.createElement('td');
        tdKod.textContent = grade.kod;
        tr.appendChild(tdKod);
        
        const tdNorm = document.createElement('td');
        tdNorm.textContent = grade.norm || '-';
        tr.appendChild(tdNorm);
        
        const tdYogunluk = document.createElement('td');
        tdYogunluk.textContent = grade.yogunluk;
        tr.appendChild(tdYogunluk);
        ...
        if (grade.custom) {
            ...
            deleteBtn.addEventListener('click', function(e) {
                e.stopPropagation();
                window.deleteMaterialGrade(grade.kod);
            });
            tdIslem.appendChild(deleteBtn);
        } else {
            const span = document.createElement('span');
...
// Focus kaybı durumunda otomatik düzeltme
        ...
        setTimeout(() => {
            ...
            if (!activeElement || activeElement === document.body || activeElement === document.documentElement) {
                ...
                if (modal && !modal.classList.contains('hidden')) {
                    const modalInput = modal.querySelector('input:not([readonly]):not([type="hidden"]), select');
                    if (modalInput) {
                        modalInput.focus();
                        modalInput.click();
                    }
                } else {
                    const mainInput = document.querySelector('#malzemeTuru');
                    if (mainInput) {
                        mainInput.focus();
                    }
                }
...
// Yeni fonksiyon: Malzeme cinsi modal içeriğini güncelleme
window.refreshMaterialGradesManager = function() {
    const modalBody = document.getElementById('settingsBody');
    const modalHeader = document.querySelector('.modal-header h2');
    
    if (modalBody && modalHeader) {
        const grades = MaterialData.getAllMaterialGrades();
        const content = generateMaterialGradesContent(grades);
        
        modalBody.innerHTML = content;
        modalHeader.textContent = 'Malzeme Cinsleri Yönetimi';

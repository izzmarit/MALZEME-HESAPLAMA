...
(function(window) {
    ...
    calculateDirsek: function() {
        ...
        if (!dirsekTipi || !dnOlcusu || !etKalinlik) {
        ...
        if (dirsekData && dirsekData[dnOlcusu] && dirsekData[dnOlcusu][etKalinlik]) {
            ...
            // Eğer tabloda yoksa formül ile hesapla
            const disCap = this.getDirsekDisCap(dnOlcusu);
            const r = dirsekTipi.includes('LR') ? disCap * 1.5 : disCap;
            const aci = parseInt(dirsekTipi) || 90;
            
            // Dirsek ağırlık formülü: π * r * (π * aci / 180) * ((D - t) * t) * yoğunluk / 1000000
            const yogunluk = MaterialData.YOGUNLUKLAR[document.getElementById('malzemeCinsi').value] || 7850;
            const hacim = Math.PI * r * (Math.PI * aci / 180) * ((disCap - parseFloat(etKalinlik)) * parseFloat(etKalinlik));
            agirlik = (hacim * yogunluk) / 1000000000;
        }
        
        const dirsekTipAciklama = MaterialData.DIRSEK_TIPLERI[dirsekTipi] || dirsekTipi;
        const olculer = `DN${dnOlcusu} ${dirsekTipAciklama} SCH${etKalinlik}`;

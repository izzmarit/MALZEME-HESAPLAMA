...
(function(window) {
    ...
    // Focus yönetimi düzeltmesi - YENİ FONKSİYON
    fixFocusIssues: function() {
                    ...
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === 1) { // Element node
                            ...
                            inputs.forEach((input) => {
                                this.setupInputFocus(input);
                            });
                            
                            // Eğer eklenen node kendisi input ise
                            if (node.matches && node.matches('input, select')) {
                                this.setupInputFocus(node);
                            }
                        }
                    });
                }
    ...
    // Input focus ayarı - YENİ FONKSİYON
    setupInputFocus: function(input) {
        // Disabled ve readonly olmayan input'lar için
        if (!input.disabled && !input.readOnly) {
            input.addEventListener('focus', function() {
                // Tıklama ile focus alındığında seç
                setTimeout(() => {

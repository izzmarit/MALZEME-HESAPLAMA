...
(function(window) {
    ...
    const FilterManager = {
        ...
        getUniqueMaterialTypes: function() {
            ...
            const types = [...new Set(data.map(row => row.malzemeTuru))];
        ...
        openFilteredView: function() {
            try {
                ...
                if (this.filteredWindow && !this.filteredWindow.closed) {
                    this.filteredWindow.close();
                }
                
                this.filteredWindow = window.open('', '_blank', 'width=1200,height=800,scrollbars=yes,resizable=yes');
                
                if (!this.filteredWindow) {
                    UIManager.showNotification('Popup engelleyicisi aktif olabilir. Lütfen popup\'lara izin verin.', 'warning');
                    return;
                }
                
                const html = this.generateFilteredHTML();
                this.filteredWindow.document.write(html);
                this.filteredWindow.document.close();
                
                // Pencere yüklendiğinde event'leri ekle
                this.filteredWindow.addEventListener('load', () => {
                    this.setupFilteredWindowEvents();
                });
                
                UIManager.showNotification(`${this.filterType} türünde ${this.filteredData.length} malzeme filtrelendi`, 'success');
                
            } catch (error) {
                console.error('Filtrelenmiş görünüm oluşturma hatası:', error);
                UIManager.showNotification('Filtrelenmiş görünüm açılamadı', 'error');
            }
        },

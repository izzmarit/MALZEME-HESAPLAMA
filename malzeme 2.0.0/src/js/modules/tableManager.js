...
(function(window) {
    ...
    deleteRow: function(index) {
        if (confirm('Bu satırı silmek istediğinizden emin misiniz?')) {
            ...
            if (this.editingIndex === index) {
                ...
                // Focus durumunu koruyarak temizle
                this.clearFormWithFocusRecovery();
            }
    ...
    // Focus durumunu koruyan temizleme fonksiyonu
    clearFormWithFocusRecovery: function() {
        // Mevcut focus durumunu kaydet
        const currentFocusElement = document.activeElement;
        const wasFocused = currentFocusElement && currentFocusElement !== document.body;
        
        // Form temizleme işlemi
        Calculator.clearForm();
        
        // Focus durumunu düzelt
        setTimeout(() => {
            // Eğer önceden bir input'a focus varsa, benzer bir input'a focus ver
            if (wasFocused) {
                const similarInput = document.querySelector('input:not([readonly]):not([disabled]), select:not([disabled])');
                if (similarInput) {
                    similarInput.focus();
                    
                    // Focus verme işlemi başarısız olursa alternatif yöntem
                    if (document.activeElement !== similarInput) {
                        similarInput.click();
                        setTimeout(() => {
                            similarInput.focus();
                            if (similarInput.select) {
                                similarInput.select();
                            }
                        }, 10);
                    }
            ...
            // Tüm input alanlarının klavye girdisini yeniden aktif et
            this.reactivateInputElements();
        }, 100);
    },

    // Input elementlerini yeniden aktif etme
    reactivateInputElements: function() {
        const allInputs = document.querySelectorAll('input, select, textarea');
        
        allInputs.forEach(input => {
            // Temel özellikleri yeniden ayarla
    ...
    // Input focus yöneticisi
    inputFocusHandler: function() {
        setTimeout(() => {
            if (this.select && this.type !== 'button' && this.type !== 'checkbox' && this.type !== 'radio') {
                this.select();
            }
        }, 10);
    },

    // Input click yöneticisi

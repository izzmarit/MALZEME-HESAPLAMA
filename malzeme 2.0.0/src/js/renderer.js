...
document.addEventListener('DOMContentLoaded', () => {
    // Focus kaybı durumunda otomatik düzeltme (mevcut kod korunuyor)
    ...
    window.addEventListener('focus', () => {
        ...
        focusCheckInterval = setInterval(() => {
            ...
            if (currentActiveElement === document.body) {
                const visibleInput = document.querySelector('input:not([readonly]):not([disabled]):not([type="hidden"]), select:not([disabled])');
        ...
        setTimeout(() => clearInterval(focusCheckInterval), 3000);
    });
    
    // Tıklama olaylarında focus kontrolü (mevcut kod korunuyor)
    ...
    // Ana sistem başlatma (32. satırdan sonraki tüm kod buraya taşınacak)
    ...
    window.MaterialCalculator = {
        ...
        malzemeTuruDegisti: function() {
            ...
            // Focus yönetimi - GÜÇLENDIRILMIŞ VERSİYON
            setTimeout(() => {
                const firstInput = olcuAlanlari.querySelector('input:not([readonly]), select');
                if (firstInput) {
                    // Elementlerin klavye girdisini zorla aktif et
                    const allNewInputs = olcuAlanlari.querySelectorAll('input, select');
                    allNewInputs.forEach(input => {
                        ...
                        input.addEventListener('focus', function() {
                            if (this.select && this.type !== 'button') {
                        ...
                        // Keydown event'ini test et
                        input.addEventListener('keydown', function(e) {
                            // Klavye girdisinin çalıştığını doğrula
                            console.log('Keyboard input working for:', this.id);
                        });
                    });

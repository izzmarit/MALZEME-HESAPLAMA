...
const EventManager = {
    ...
    handleClick: function(e) {
        if (e.target.matches('input:not([readonly]), select:not([disabled]), textarea:not([readonly])')) {
            this.ensureInputFunctionality(e.target);
        }
    },
    ...
    handleFocus: function(e) {
        if (e.target.matches('input, select, textarea')) {
            ...
            // Text selection için
            if (e.target.select && e.target.type !== 'button' && e.target.type !== 'checkbox') {
                setTimeout(() => {
                    try {
                        e.target.select();
                    } catch (err) {
                        // Bazı input tipleri select desteklemez
                    }
                }, 10);
            }
        }
    ...
    handleWindowFocus: function() {
        // Window focus geri geldiğinde input'ları yeniden aktive et
        setTimeout(() => {
            this.reactivateAllInputs();
        }, 100);
    },

    ensureInputFunctionality: function(input) {
        if (this.activeListeners.has(input)) {
            return; // Zaten aktif
        }

        // Input'un temel özelliklerini garanti et
        input.tabIndex = input.tabIndex || 0;
        input.style.pointerEvents = 'auto';
        input.style.userSelect = 'auto';
        
        // Keydown listener ekle
        const keydownHandler = function(e) {
            // Klavye girişini garanti et
            e.stopPropagation();
        };
        
        input.addEventListener('keydown', keydownHandler, { passive: true });
        
        // Input'un aktif olduğunu işaretle
        this.activeListeners.set(input, {
    ...
    reactivateAllInputs: function() {
        ...
        inputs.forEach(input => {
            this.ensureInputFunctionality(input);
        });
    },
            ...
            mutations.forEach((mutation) => {
                if (mutation.type === 'childList') {
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === 1) {
                            // Yeni eklenen input'ları bul ve aktive et
                            const inputs = node.querySelectorAll ? 
                                node.querySelectorAll('input:not([readonly]), select:not([disabled]), textarea:not([readonly])') : [];
                            
                            inputs.forEach(input => this.ensureInputFunctionality(input));
                            
                            if (node.matches && node.matches('input:not([readonly]), select:not([disabled]), textarea:not([readonly])')) {
                                this.ensureInputFunctionality(node);
                            }
                        }
                    });
                }
    ...
    clearInput: function(input) {
        // Input'tan event listener'ları temizle
        const listeners = this.activeListeners.get(input);
        if (listeners) {
            input.removeEventListener('keydown', listeners.keydownHandler);
            this.activeListeners.delete(input);
        }
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.MaterialCalculator = {
                ...
                <div class="material-add-container">
                    ...
                    <div id="birim-panel" class="material-panel">
                        ...
                        <div class="material-form-group">
                            ...
                            <input type="text" id="birimAgirlik" class="material-input" placeholder="0.000" pattern="[0-9]*\.?[0-9]*">
                        </div>
        ...
        saveBirimTabanliMalzeme: function() {
            ...
            const birimAgirlikElement = document.getElementById('birimAgirlik');
            ...
            // Number input için özel değer alma yöntemi
            let birimAgirlik = '';
            if (birimAgirlikElement) {
                // Önce string olarak al
                const rawValue = birimAgirlikElement.value;
                console.log('Ham değer (rawValue):', rawValue);
                
                // Değeri kontrol et
                if (rawValue !== null && rawValue !== undefined && rawValue !== '') {
                    birimAgirlik = rawValue.toString();
                }
            }
